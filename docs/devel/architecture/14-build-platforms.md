...
([`mk/tests.mk#test`](../../../mk/tests.mk#L415)) builds **only** `helix-tests`; `make test-run` ([`mk/tests.mk`](../../../mk/tests.mk#L418)) builds it and
...
([`mk/fonts.mk#regen-fonts`](../../../mk/fonts.mk#L179)), `make translations` / `make translation-sync` for the YAML string pipeline
...
[`mk/cross.mk`](../../../mk/cross.mk#L741)). `make help` prints the target menu; `make help-all` adds the test, cross, and
remote groups.
...
([`mk/display-lib.mk`](../../../mk/display-lib.mk#L23): fbdev+DRM always on Linux, SDL when `ENABLE_SDL`) but runtime *selection*:
...
CFS, and IFS gates are off on AD5M ([`mk/cross.mk`](../../../mk/cross.mk#L240)). The ESP32 port (`firmware/helixscreen-esp32/`,
...
([`mk/patches.mk`](../../../mk/patches.mk#L195)): a `patches/*.patch` with no apply block in [`mk/patches.mk`](../../../mk/patches.mk), or an apply block
...
- **Test builds reach the patch stamp only through the PCH prerequisite** ([`mk/rules.mk`](../../../mk/rules.mk#L214)); the `test` target does not itself gate on `apply-patches`. After a patch red-line or submodule bump, run `make -j` or `make reapply-patches` — don't assume `make test-run` re-verified the tree (#1212).
...
4. [`mk/tests.mk#test`](../../../mk/tests.mk#L415) — the `test` (build-only) vs `test-run` (parallel shards) split, and the `~[.] ~[slow]` filter convention.
5. [`mk/cross.mk`](../../../mk/cross.mk#L8) — the commented platform menu; then `:58` (pi: DRM+GLES, all font tiers) against `:216` (ad5m: `-Os -flto -static`, label-printer gate off, trimmed fonts) to see how far the knobs turn.
6. [`mk/cross.mk`](../../../mk/cross.mk#L681) — the `native` block: SDL backend, and why dev conveniences live here rather than in cross builds.
...
8. [`mk/display-lib.mk`](../../../mk/display-lib.mk#L23) — compile-time backend inclusion per OS (Darwin gets SDL only; Linux always gets fbdev+DRM).
