...
6. **`ObserverGuard::invalidate_all()`** ([`src/application/application.cpp:5255`](../../../src/application/application.cpp#L5255)), then destroy MoonrakerManager, stop the HttpExecutors, `m_display.reset()` → `DisplayManager::shutdown()` which runs **`lv_deinit()` then `lv_xml_deinit()`** ([`src/application/display_manager.cpp:715`](../../../src/application/display_manager.cpp#L715)), and finally `theme_manager_deinit()` dead last ([`src/application/application.cpp:5297`](../../../src/application/application.cpp#L5297)).
