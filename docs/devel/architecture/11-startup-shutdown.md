...
6. **`ObserverGuard::invalidate_all()`** ([`src/application/application.cpp:5252`](../../../src/application/application.cpp#L5252)), then destroy MoonrakerManager, stop the HttpExecutors, `m_display.reset()` → `DisplayManager::shutdown()` which runs **`lv_deinit()` then `lv_xml_deinit()`** ([`src/application/display_manager.cpp:715`](../../../src/application/display_manager.cpp#L715)), and finally `theme_manager_deinit()` dead last ([`src/application/application.cpp:5294`](../../../src/application/application.cpp#L5294)).
...
7. [`src/application/application.cpp:3635`](../../../src/application/application.cpp#L3635) — `connect_moonraker()`: when it connects, what Safe Mode skips, where `start_auto_send` lands.
...
9. [`src/application/application.cpp:4030`](../../../src/application/application.cpp#L4030) — `main_loop()`: read only the splash-handoff and 11s-failsafe blocks (`:4154`–`:4205`); chapter 02 owns the rest.
10. [`src/application/application.cpp:5050`](../../../src/application/application.cpp#L5050) — `shutdown()`: walk the ladder against the "How it works" list; the long comments at `:5158` and `:5219` are the two UAF post-mortems.
