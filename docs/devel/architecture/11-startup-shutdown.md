...
The boot ladder, numbered exactly as the phase comments in `run()` order them ([`src/application/application.cpp:519`](../../../src/application/application.cpp#L519)):
...
Phase 8c (`register_xml_components`, `:1790`) calls `helix::register_xml_components()` ([`src/xml_registration.cpp:303`](../../../src/xml_registration.cpp#L303)), which registers responsive constants, the semantic text/button widgets, shared event callbacks, then 305 XML component files (`register_xml()` call sites, recounted at audit) through `LayoutManager::resolve_xml_path()` (variant-aware), yielding every 16 components on ESP32 so the idle task is not starved. If `HELIX_HOT_RELOAD` is on (default for native builds), the hot-reloader thread starts here too.
...
Phase 9a constructs `SubjectInitializer` and runs `init_core_and_state()` ([`src/application/subject_initializer.cpp:228`](../../../src/application/subject_initializer.cpp#L228)): core globals → `PrinterState` → AMS/sensor managers → `NavigationManager` last, *precisely so* reverse-order deinit clears NavigationManager's observers before PrinterState frees the subjects they observe. Phase 9b starts the HttpExecutor pools ([`src/application/application.cpp:778`](../../../src/application/application.cpp#L778) — the Moonraker APIs submit to them on every request) before `MoonrakerManager::init()` creates the client and API. Then come the system services the panels will need: `UpdateChecker`, `UpgradeBanner`, `CrashHistory`/`CrashReporter`, `TelemetryManager::init()` ([`src/application/application.cpp:808`](../../../src/application/application.cpp#L808)) — note the telemetry spread: init here, `record_session()` on discovery complete ([`src/application/application.cpp:3397`](../../../src/application/application.cpp#L3397)), `start_auto_send()` ([`src/application/application.cpp:3749`](../../../src/application/application.cpp#L3749)) inside `connect_moonraker()`.
...
4. **`StaticSubjectRegistry::deinit_all()`** (`:5153`) — LIFO over self-registered `deinit_subjects()` callbacks ([`src/application/static_subject_registry.cpp:67`](../../../src/application/static_subject_registry.cpp#L67) iterates a detached copy in reverse, so a callback that re-registers lands in the empty member vector).
...
6. **`ObserverGuard::invalidate_all()`** ([`src/application/application.cpp:5303`](../../../src/application/application.cpp#L5303)), then destroy MoonrakerManager, stop the HttpExecutors, `m_display.reset()` → `DisplayManager::shutdown()` which runs **`lv_deinit()` then `lv_xml_deinit()`** ([`src/application/display_manager.cpp:715`](../../../src/application/display_manager.cpp#L715)), and finally `theme_manager_deinit()` dead last ([`src/application/application.cpp:5345`](../../../src/application/application.cpp#L5345)).
...
- **Phase numbers are comments in `run()`, not an enum** — "Phase 9d" only exists in a comment at [`application.cpp:825`](../../../src/application/application.cpp#L825). When you add a step, place it in the comment ladder; the logs and this chapter quote those numbers.
...
2. [`src/application/application.cpp:519`](../../../src/application/application.cpp#L519) — `run()`: skim top-to-bottom once reading only the `Phase N` comments; this is the authoritative ladder.
3. [`src/application/application.cpp:1396`](../../../src/application/application.cpp#L1396) — `init_display()`: DPI forcing, and the splash suppression block at `:1576` (invalidation off + no-op flush callback).
4. [`src/application/application.cpp:1658`](../../../src/application/application.cpp#L1658) — `init_theme()`: note [`globals.xml`](../../../ui_xml/globals.xml) registered *before* `theme_manager_init()` and why.
5. [`src/xml_registration.cpp:303`](../../../src/xml_registration.cpp#L303) — `register_xml_components()`: responsive consts → semantic widgets → callbacks → XML files; the `boot_yield` cadence note at `:290`.
6. [`src/application/subject_initializer.cpp:228`](../../../src/application/subject_initializer.cpp#L228) — `init_core_and_state()`: the dependency phases, and the NavigationManager-registers-last comment.
7. [`src/application/application.cpp:3684`](../../../src/application/application.cpp#L3684) — `connect_moonraker()`: when it connects, what Safe Mode skips, where `start_auto_send` lands.
8. [`src/application/application.cpp:2020`](../../../src/application/application.cpp#L2020) — `init_ui()`: the timed `lv_xml_create`, navbar wiring, `PanelFactory` handoff.
9. [`src/application/application.cpp:4079`](../../../src/application/application.cpp#L4079) — `main_loop()`: read only the splash-handoff and 11s-failsafe blocks (`:4154`–`:4205`); chapter 02 owns the rest.
10. [`src/application/application.cpp:5099`](../../../src/application/application.cpp#L5099) — `shutdown()`: walk the ladder against the "How it works" list; the long comments at `:5158` and `:5219` are the two UAF post-mortems.
11. [`src/application/display_manager.cpp:644`](../../../src/application/display_manager.cpp#L644) — `DisplayManager::shutdown()`: `lv_deinit()` → `lv_xml_deinit()` and the style-ownership comment above them.
