...
- **Phase numbers are comments in `run()`, not an enum** — "Phase 9d" only exists in a comment at [`application.cpp:816`](../../../src/application/application.cpp#L816). When you add a step, place it in the comment ladder; the logs and this chapter quote those numbers.
...
3. [`src/application/application.cpp:1389`](../../../src/application/application.cpp#L1389) — `init_display()`: DPI forcing, and the splash suppression block at `:1576` (invalidation off + no-op flush callback).
4. [`src/application/application.cpp:1651`](../../../src/application/application.cpp#L1651) — `init_theme()`: note [`globals.xml`](../../../ui_xml/globals.xml) registered *before* `theme_manager_init()` and why.
...
8. [`src/application/application.cpp:2014`](../../../src/application/application.cpp#L2014) — `init_ui()`: the timed `lv_xml_create`, navbar wiring, `PanelFactory` handoff.
