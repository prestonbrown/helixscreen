...
6. **`ObserverGuard::invalidate_all()`** (`:5206`), then destroy MoonrakerManager, stop the HttpExecutors, `m_display.reset()` → `DisplayManager::shutdown()` which runs **`lv_deinit()` then `lv_xml_deinit()`** ([`src/application/display_manager.cpp:703`](../../../src/application/display_manager.cpp#L703)), and finally `theme_manager_deinit()` dead last (`:5247`).
...
10. [`src/application/application.cpp:5027`](../../../src/application/application.cpp#L5027) — `shutdown()`: walk the ladder against the "How it works" list; the long comments at `:5173` and `:5208` are the two UAF post-mortems.
