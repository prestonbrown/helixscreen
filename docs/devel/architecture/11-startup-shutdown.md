...
The boot ladder, numbered exactly as the phase comments in `run()` order them ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L518)):
...
Phase 8c (`register_xml_components`, `:1790`) calls `helix::register_xml_components()` ([`src/xml_registration.cpp#register_xml_components`](../../../src/xml_registration.cpp#L300)), which registers responsive constants, the semantic text/button widgets, shared event callbacks, then 305 XML component files (`register_xml()` call sites, recounted at audit) through `LayoutManager::resolve_xml_path()` (variant-aware), yielding every 16 components on ESP32 so the idle task is not starved. If `HELIX_HOT_RELOAD` is on (default for native builds), the hot-reloader thread starts here too.
...
Phase 9a constructs `SubjectInitializer` and runs `init_core_and_state()` ([`src/application/subject_initializer.cpp#init_core_and_state`](../../../src/application/subject_initializer.cpp#L228)): core globals → `PrinterState` → AMS/sensor managers → `NavigationManager` last, *precisely so* reverse-order deinit clears NavigationManager's observers before PrinterState frees the subjects they observe. Phase 9b starts the HttpExecutor pools ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L777) — the Moonraker APIs submit to them on every request) before `MoonrakerManager::init()` creates the client and API. Then come the system services the panels will need: `UpdateChecker`, `UpgradeBanner`, `CrashHistory`/`CrashReporter`, `TelemetryManager::init()` ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L807)) — note the telemetry spread: init here, `record_session()` on discovery complete ([`src/application/application.cpp#setup_discovery_callbacks`](../../../src/application/application.cpp#L3350)), `start_auto_send()` ([`src/application/application.cpp#connect_moonraker`](../../../src/application/application.cpp#L3702)) inside `connect_moonraker()`.
...
4. **`StaticSubjectRegistry::deinit_all()`** (`:5153`) — LIFO over self-registered `deinit_subjects()` callbacks ([`src/application/static_subject_registry.cpp#deinit_all`](../../../src/application/static_subject_registry.cpp#L54) iterates a detached copy in reverse, so a callback that re-registers lands in the empty member vector).
...
6. **`ObserverGuard::invalidate_all()`** ([`src/application/application.cpp#shutdown`](../../../src/application/application.cpp#L5254)), then destroy MoonrakerManager, stop the HttpExecutors, `m_display.reset()` → `DisplayManager::shutdown()` which runs **`lv_deinit()` then `lv_xml_deinit()`** ([`src/application/display_manager.cpp#shutdown`](../../../src/application/display_manager.cpp#L715)), and finally `theme_manager_deinit()` dead last ([`src/application/application.cpp#shutdown`](../../../src/application/application.cpp#L5296)).
...
- **Phase numbers are comments in `run()`, not an enum** — "Phase 9d" only exists in a comment at [`src/application/application.cpp#run`](../../../src/application/application.cpp#L824). When you add a step, place it in the comment ladder; the logs and this chapter quote those numbers.
...
2. [`src/application/application.cpp#run`](../../../src/application/application.cpp#L518) — `run()`: skim top-to-bottom once reading only the `Phase N` comments; this is the authoritative ladder.
3. [`src/application/application.cpp#init_display`](../../../src/application/application.cpp#L1395) — `init_display()`: DPI forcing, and the splash suppression block at `:1576` (invalidation off + no-op flush callback).
4. [`src/application/application.cpp#init_theme`](../../../src/application/application.cpp#L1613) — `init_theme()`: note [`globals.xml`](../../../ui_xml/globals.xml) registered *before* `theme_manager_init()` and why.
5. [`src/xml_registration.cpp#register_xml_components`](../../../src/xml_registration.cpp#L300) — `register_xml_components()`: responsive consts → semantic widgets → callbacks → XML files; the `boot_yield` cadence note at `:290`.
...
7. [`src/application/application.cpp#connect_moonraker`](../../../src/application/application.cpp#L3637) — `connect_moonraker()`: when it connects, what Safe Mode skips, where `start_auto_send` lands.
8. [`src/application/application.cpp#init_ui`](../../../src/application/application.cpp#L1975) — `init_ui()`: the timed `lv_xml_create`, navbar wiring, `PanelFactory` handoff.
9. [`src/application/application.cpp#main_loop`](../../../src/application/application.cpp#L4032) — `main_loop()`: read only the splash-handoff and 11s-failsafe blocks (`:4154`–`:4205`); chapter 02 owns the rest.
10. [`src/application/application.cpp#shutdown`](../../../src/application/application.cpp#L5052) — `shutdown()`: walk the ladder against the "How it works" list; the long comments at `:5158` and `:5219` are the two UAF post-mortems.
...
12. [`src/application/static_subject_registry.cpp#deinit_all`](../../../src/application/static_subject_registry.cpp#L54) — `deinit_all()`: 25 lines; the detached-copy/reverse-iteration trick.
13. [`src/helix_watchdog.cpp#run_watchdog`](../../../src/helix_watchdog.cpp#L1099) — `run_watchdog()`: fork/supervise loop, exit-code translation at `:1221`, crash-loop branch at `:1336`.
...
16. [`scripts/helix-launcher.sh`](../../../scripts/helix-launcher.sh#L452) — how watchdog and splash get started on device, and the `HELIX_NO_SPLASH`/pre-started-splash branches.
