...
6. **`ObserverGuard::invalidate_all()`** ([`src/application/application.cpp:5251`](../../../src/application/application.cpp#L5251)), then destroy MoonrakerManager, stop the HttpExecutors, `m_display.reset()` → `DisplayManager::shutdown()` which runs **`lv_deinit()` then `lv_xml_deinit()`** ([`src/application/display_manager.cpp:715`](../../../src/application/display_manager.cpp#L715)), and finally `theme_manager_deinit()` dead last ([`src/application/application.cpp:5293`](../../../src/application/application.cpp#L5293)).
...
3. [`src/application/application.cpp:1395`](../../../src/application/application.cpp#L1395) — `init_display()`: DPI forcing, and the splash suppression block at `:1576` (invalidation off + no-op flush callback).
4. [`src/application/application.cpp:1613`](../../../src/application/application.cpp#L1613) — `init_theme()`: note [`globals.xml`](../../../ui_xml/globals.xml) registered *before* `theme_manager_init()` and why.
...
8. [`src/application/application.cpp:1975`](../../../src/application/application.cpp#L1975) — `init_ui()`: the timed `lv_xml_create`, navbar wiring, `PanelFactory` handoff.
9. [`src/application/application.cpp:4029`](../../../src/application/application.cpp#L4029) — `main_loop()`: read only the splash-handoff and 11s-failsafe blocks (`:4154`–`:4205`); chapter 02 owns the rest.
10. [`src/application/application.cpp:5049`](../../../src/application/application.cpp#L5049) — `shutdown()`: walk the ladder against the "How it works" list; the long comments at `:5158` and `:5219` are the two UAF post-mortems.
