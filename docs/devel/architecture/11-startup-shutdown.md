...
6. **`ObserverGuard::invalidate_all()`** ([`src/application/application.cpp:5247`](../../../src/application/application.cpp#L5247)), then destroy MoonrakerManager, stop the HttpExecutors, `m_display.reset()` → `DisplayManager::shutdown()` which runs **`lv_deinit()` then `lv_xml_deinit()`** ([`src/application/display_manager.cpp:703`](../../../src/application/display_manager.cpp#L703)), and finally `theme_manager_deinit()` dead last ([`src/application/application.cpp:5289`](../../../src/application/application.cpp#L5289)).
...
7. [`src/application/application.cpp:3630`](../../../src/application/application.cpp#L3630) — `connect_moonraker()`: when it connects, what Safe Mode skips, where `start_auto_send` lands.
...
9. [`src/application/application.cpp:4025`](../../../src/application/application.cpp#L4025) — `main_loop()`: read only the splash-handoff and 11s-failsafe blocks (`:4154`–`:4205`); chapter 02 owns the rest.
10. [`src/application/application.cpp:5045`](../../../src/application/application.cpp#L5045) — `shutdown()`: walk the ladder against the "How it works" list; the long comments at `:5158` and `:5219` are the two UAF post-mortems.
