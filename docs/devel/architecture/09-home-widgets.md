...
3. [`src/ui/panel_widget_registry.cpp:88`](../../../src/ui/panel_widget_registry.cpp#L88) — the def table itself: spot the gate subjects, the span triples (min/default/max), the `multi_instance` column, and the `HELIX_HAS_CAMERA` guard; then `init_widget_registrations()` at `:147` and the `:N` suffix strip in `find_widget_def()` at `:110`.
