...
- `on_size_changed(colspan, rowspan, width_px, height_px)` — adapt content to the cell; called by the manager right after every `attach()` ([`panel_widget_manager.cpp:1216`](../../../src/ui/panel_widget_manager.cpp#L1216)), including on reused instances.
...
- **`on_size_changed` tiers should key on pixels, not spans** — the same span is a different physical width on every panel size; `FanStackWidget` derives compact-vs-normal from `width_px` against `widget_size::w_normal()` ([`fan_stack_widget.cpp:257`](../../../src/ui/panel_widgets/fan_stack_widget.cpp#L257)-284).
...
5. [`src/ui/panel_widget_manager.cpp:1295`](../../../src/ui/panel_widget_manager.cpp#L1295) — `setup_gate_observers()`: the registry walk for distinct gate subjects, the `GateRebuildSlot` coalescing (queue once, `pending` until the trampoline runs), and `clear_gate_observers()` canceling in-flight rebuilds at `:1128`.
