...
`HomePanel` owns the page dimension: a carousel of per-page containers (`build_carousel`, [`src/ui/ui_panel_home.cpp#build_carousel`](../../../src/ui/ui_panel_home.cpp#L148)), each populated independently, with `on_activate()`/`on_deactivate()` fanned to the active page's widgets on page change (`src/ui/ui_panel_home.cpp#on_page_changed`).
...
4. [`src/ui/panel_widget_manager.cpp#populate_widgets`](../../../src/ui/panel_widget_manager.cpp#L211) — `populate_widgets()` top to bottom once: slot resolution and the per-widget try/catch (`src/ui/panel_widget_manager.cpp#"configuration failed"`), the `~gated` short-circuit key (`src/ui/panel_widget_manager.cpp#"Widget list unchanged"`), `safe_clean_children` (`src/ui/panel_widget_manager.cpp#"safe_clean_children(container);"`) + `LV_LAYOUT_NONE` (`src/ui/panel_widget_manager.cpp#"lv_obj_set_layout(container, LV_LAYOUT_NONE);"`), the two auto-place policies (`src/ui/panel_widget_manager.cpp#"run_auto_pass = [&]"`), the write-back rules (`src/ui/panel_widget_manager.cpp#"Never write back OR persist a layout"`), and the create/attach loop with the tile flag (`src/ui/panel_widget_manager.cpp#"lv_obj_add_flag(widget, PANEL_WIDGET_TILE_FLAG);"`) and grid-last activation (`src/ui/panel_widget_manager.cpp#"lv_obj_set_layout(container, LV_LAYOUT_GRID);"`).
