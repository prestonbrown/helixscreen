...
Modals are not on this stack at all. `Modal::show("component_name")` creates a C++ backdrop plus dialog from an XML component and tracks the pair in `ModalStack` ([`include/ui_modal.h:389`](../../../include/ui_modal.h#L389)) - its own z-ordered stack with entrance/exit animations, backdrop-click and ESC handling, and an `exiting` flag so a second `hide()` during the exit animation is ignored. An overlay can therefore sit under a modal under another modal without `NavigationManager` knowing anything changed.
...
For the six roots this order lives in boot. `SubjectInitializer` runs each panel's `init_subjects()` ([`src/application/subject_initializer.cpp:361`](../../../src/application/subject_initializer.cpp#L361)-379) before the single root `lv_xml_create`, then `PanelFactory::setup_panels()` ([`src/application/panel_factory.cpp:129`](../../../src/application/panel_factory.cpp#L129)) finds the six widgets by name, runs `setup()`, and `activate_initial_panel()` fires the first `on_activate()` (`:142`). For everything else it happens lazily at first open (below).
...
`dismiss_token` when the capture can outlive its owner) and a close by backdrop tap, ESC or
...
- **`Modal` has both a static and an instance `show()`.** Static for XML-only, subclass instance when you need `on_ok()` logic. An instance-backed modal is hidden, never rebuilt, by hot reload - re-creating it from XML would skip `on_show()` wiring ([`include/ui_modal.h:188`](../../../include/ui_modal.h#L188)).
...
- **Shutdown ordering is fixed.** `Application::shutdown()` calls `NavigationManager::shutdown()` ([`src/application/application.cpp:5107`](../../../src/application/application.cpp#L5107)) before `StaticPanelRegistry::destroy_all()`, deactivating the visible surface first; overlays check `is_shutting_down()` and skip destructive actions (e.g. ABORT) on the way out.
...
- **`push_overlay(root, false)` — the `hide_previous` variant — is for see-through stacking.** Exactly two call sites use it: the on-screen keypad ([`src/ui/ui_component_keypad.cpp:195`](../../../src/ui/ui_component_keypad.cpp#L195), a transparent overlay over the content being edited) and the widget catalog ([`src/ui/ui_widget_catalog_overlay.cpp:392`](../../../src/ui/ui_widget_catalog_overlay.cpp#L392), which keeps the home grid visible behind the catalog so you can drag a widget out of the list). Both are function-based overlays registered with a null lifecycle; both stay visible over what they cover.
...
14. [`src/ui/ui_panel_home.cpp:815`](../../../src/ui/ui_panel_home.cpp#L815) — a `PanelBase` subclass (AMS) opened as an overlay through the same dance by hand; compare with what the helper automates.
...
16. [`src/application/subject_initializer.cpp:361`](../../../src/application/subject_initializer.cpp#L361) — the roots' `init_subjects()` calls, before any `lv_xml_create` — the ordering that makes bindings resolve.
...
19. [`include/ui_info_qr_modal.h:11`](../../../include/ui_info_qr_modal.h#L11) — the 42-line subclass; then [`src/ui/ui_info_qr_modal.cpp:13`](../../../src/ui/ui_info_qr_modal.cpp#L13) for `show_modal()`, `on_show()` wiring, and the self-deleting `on_hide()` at `:30`.
