...
- **`push_overlay(root, false)` — the `hide_previous` variant — is for see-through stacking.** Exactly two call sites use it: the on-screen keypad ([`src/ui/ui_component_keypad.cpp:195`](../../../src/ui/ui_component_keypad.cpp#L195), a transparent overlay over the content being edited) and the widget catalog ([`src/ui/ui_widget_catalog_overlay.cpp:621`](../../../src/ui/ui_widget_catalog_overlay.cpp#L621), which keeps the home grid visible behind the catalog so you can drag a widget out of the list). Both are function-based overlays registered with a null lifecycle; both stay visible over what they cover.
