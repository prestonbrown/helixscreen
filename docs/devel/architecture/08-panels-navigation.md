...
- **Shutdown ordering is fixed.** `Application::shutdown()` calls `NavigationManager::shutdown()` ([`src/application/application.cpp:5156`](../../../src/application/application.cpp#L5156)) before `StaticPanelRegistry::destroy_all()`, deactivating the visible surface first; overlays check `is_shutting_down()` and skip destructive actions (e.g. ABORT) on the way out.
...
- **`push_overlay(root, false)` — the `hide_previous` variant — is for see-through stacking.** Exactly two call sites use it: the on-screen keypad ([`src/ui/ui_component_keypad.cpp:195`](../../../src/ui/ui_component_keypad.cpp#L195), a transparent overlay over the content being edited) and the widget catalog ([`src/ui/ui_widget_catalog_overlay.cpp:619`](../../../src/ui/ui_widget_catalog_overlay.cpp#L619), which keeps the home grid visible behind the catalog so you can drag a widget out of the list). Both are function-based overlays registered with a null lifecycle; both stay visible over what they cover.
...
17. [`src/ui/ui_panel_print_status.cpp:1345`](../../../src/ui/ui_panel_print_status.cpp#L1345) — persistent registration, third argument `true`: the one overlay family that survives navbar switches.
