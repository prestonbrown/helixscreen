...
- **Shutdown ordering is fixed.** `Application::shutdown()` calls `NavigationManager::shutdown()` ([`src/application/application.cpp:5160`](../../../src/application/application.cpp#L5160)) before `StaticPanelRegistry::destroy_all()`, deactivating the visible surface first; overlays check `is_shutting_down()` and skip destructive actions (e.g. ABORT) on the way out.
...
17. [`src/ui/ui_panel_print_status.cpp:1420`](../../../src/ui/ui_panel_print_status.cpp#L1420) — persistent registration, third argument `true`: the one overlay family that survives navbar switches.
