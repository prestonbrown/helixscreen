...
2. [`include/printer_discovery.h#PrinterDiscovery`](../../../include/printer_discovery.h#L41) — the class doc listing everything one snapshot holds; skim `parse_objects()` at `:52` far enough to see the chamber-heater scoring lambdas (backend-registry consult plus object-type tiebreak).
...
7. [`src/application/application.cpp#setup_discovery_callbacks`](../../../src/application/application.cpp#L3211) — the self-guard comment, then the `auto_detect_and_save()` call at `:3171`; scroll up to `:2970` for `set_hardware()` on the same path.
8. [`include/printer_capabilities_state.h#PrinterCapabilitiesState`](../../../include/printer_capabilities_state.h#L370) — all 22 subjects in one block; then `set_hardware()`'s contract at `:59` and the `AsyncLifetimeGuard` comment at `:328`.
...
11. [`src/ui/panel_widget_manager.cpp#setup_gate_observers`](../../../src/ui/panel_widget_manager.cpp#L1302) — `setup_gate_observers()`: one observer per distinct gate subject, coalesced rebuild; cross-reference the gate check at `:128`-137.
...
16. [`src/printer/z_offset_persistence.cpp#Provider`](../../../src/printer/z_offset_persistence.cpp#L15) — the `Provider` table (one row), then the schema-read rationale at `:94`; finish with the call sites: [`src/api/moonraker_discovery_sequence.cpp#build_subscription_objects`](../../../src/api/moonraker_discovery_sequence.cpp#L1398), [`src/printer/printer_motion_state.cpp#update_from_status`](../../../src/printer/printer_motion_state.cpp#L206), [`src/application/application.cpp#setup_discovery_callbacks`](../../../src/application/application.cpp#L3070).
...
18. [`include/z_offset_utils.h#helix::zoffset`](../../../include/z_offset_utils.h#L26) — the enum's consumers: `is_auto_saved()` at `:18`, then `apply_and_save()` at `:76` taking the API and the strategy — generic calibration code that never learns which printer it is talking to.
