...
Chapter 04 ended the wire story at `MoonrakerDiscoverySequence`; this chapter starts at its output. The sequence classifies `printer.objects.list` into `PrinterDiscovery` ([`include/printer_discovery.h:40`](../../../include/printer_discovery.h#L40)) — a by-value snapshot holding heaters, sensors, fans, LEDs, steppers, MCUs, kinematics, build volume, macro names, and AMS/filament-system objects — guarded by a mutex and handed out as a copy via `hardware()` ([`include/moonraker_discovery_sequence.h:104`](../../../include/moonraker_discovery_sequence.h#L104)). Two consumers meet it during startup: `PrinterState::set_hardware()` ([`src/application/application.cpp:2993`](../../../src/application/application.cpp#L2993)) pushes the snapshot into state, and `PrinterDetector::auto_detect_and_save()` ([`src/application/application.cpp:3174`](../../../src/application/application.cpp#L3174)) tries to name the printer.
...
One honesty note the older documentation obscures: `SensorRegistry` ([`include/sensor_registry.h:60`](../../../include/sensor_registry.h#L60)) — the "central registry" that fans `discover_all()`/`update_all_from_status()` across managers — is never instantiated in production; its only constructions are its own unit tests ([`tests/unit/test_sensor_registry.cpp`](../../../tests/unit/test_sensor_registry.cpp), 15 of them). The live wiring is direct: `PrinterDiscovery::parse_objects()` calls five managers' `discover()` itself ([`src/printer/printer_discovery.cpp:105`](../../../src/printer/printer_discovery.cpp#L105)-131), the discovery sequence's `configfile.config` step queues `ProbeSensorManager` and `AccelSensorManager` `discover_from_config()` to the main thread ([`src/api/moonraker_discovery_sequence.cpp:883`](../../../src/api/moonraker_discovery_sequence.cpp#L883)-775 — accel sensors appear *only* there), and status frames fan out to all seven from `PrinterState::update_from_status()` ([`src/printer/printer_state.cpp:638`](../../../src/printer/printer_state.cpp#L638)-646). The mock seeds the same managers directly ([`src/api/moonraker_client_mock.cpp:787`](../../../src/api/moonraker_client_mock.cpp#L787), `:906`), so `--test` exercises identical code paths.
...
|-------------------|------|--------------------------------------|
| subscription builder ([`src/api/moonraker_discovery_sequence.cpp:1491`](../../../src/api/moonraker_discovery_sequence.cpp#L1491)) | `zoffset::required_status_objects(hw)` | `if (hw.has_macro("SAVE_ZMOD_DATA")) subs["save_variables"] = nullptr;` |
...
- **The `is_*_printer()` helpers read the saved `printer.type` string, not live discovery** (`is_voron_printer()`, `is_creality_k1()`, `is_pfa_printer()`, ... at [`include/printer_detector.h:494`](../../../include/printer_detector.h#L494)+). They exist for rendering picks (toolhead style) and are timing-unsafe if treated as live capability answers early in boot — a fresh install has not written `printer.type` yet, and the CFS backend's K1-vs-K2 dialect check relies on Config being populated from a prior run (#1053 documented why that migration is a won't-do).
...
2. [`include/printer_discovery.h:40`](../../../include/printer_discovery.h#L40) — the class doc listing everything one snapshot holds; skim `parse_objects()` at `:52` far enough to see the chamber-heater scoring lambdas (keyword tier plus object-type tiebreak).
3. [`include/printer_detector.h:121`](../../../include/printer_detector.h#L121) — the static class; read `detect()`'s contract at `:111`, then `apply_preset_with_variants()` at `:192` for the firmware-variant trick, and `LoadStatus` at `:375`.
...
7. [`src/application/application.cpp:3162`](../../../src/application/application.cpp#L3162) — the self-guard comment, then the `auto_detect_and_save()` call at `:3171`; scroll up to `:2970` for `set_hardware()` on the same path.
8. [`include/printer_capabilities_state.h:354`](../../../include/printer_capabilities_state.h#L354) — all 22 subjects in one block; then `set_hardware()`'s contract at `:59` and the `AsyncLifetimeGuard` comment at `:328`.
...
11. [`src/ui/panel_widget_manager.cpp:1026`](../../../src/ui/panel_widget_manager.cpp#L1026) — `setup_gate_observers()`: one observer per distinct gate subject, coalesced rebuild; cross-reference the gate check at `:128`-137.
...
14. [`src/printer/printer_state.cpp:638`](../../../src/printer/printer_state.cpp#L638)-646 — the status fan-out to all seven managers — the sensor framework's live-data path.
15. [`include/z_offset_persistence.h:33`](../../../include/z_offset_persistence.h#L33)-63 — the whole vendor abstraction: five functions, none naming a firmware in its signature.
16. [`src/printer/z_offset_persistence.cpp:15`](../../../src/printer/z_offset_persistence.cpp#L15) — the `Provider` table (one row), then the schema-read rationale at `:94`; finish with the call sites: [`src/api/moonraker_discovery_sequence.cpp:1394`](../../../src/api/moonraker_discovery_sequence.cpp#L1394), [`src/printer/printer_motion_state.cpp:206`](../../../src/printer/printer_motion_state.cpp#L206), [`src/application/application.cpp:3021`](../../../src/application/application.cpp#L3021).
