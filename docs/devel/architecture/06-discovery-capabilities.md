...
One honesty note the older documentation obscures: `SensorRegistry` ([`include/sensor_registry.h:60`](../../../include/sensor_registry.h#L60)) — the "central registry" that fans `discover_all()`/`update_all_from_status()` across managers — is never instantiated in production; its only constructions are its own unit tests ([`tests/unit/test_sensor_registry.cpp`](../../../tests/unit/test_sensor_registry.cpp), 15 of them). The live wiring is direct: `PrinterDiscovery::parse_objects()` calls five managers' `discover()` itself ([`src/printer/printer_discovery.cpp:105`](../../../src/printer/printer_discovery.cpp#L105)-131), the discovery sequence's `configfile.config` step queues `ProbeSensorManager` and `AccelSensorManager` `discover_from_config()` to the main thread ([`src/api/moonraker_discovery_sequence.cpp:887`](../../../src/api/moonraker_discovery_sequence.cpp#L887)-779 — accel sensors appear *only* there), and status frames fan out to all seven from `PrinterState::update_from_status()` ([`src/printer/printer_state.cpp:641`](../../../src/printer/printer_state.cpp#L641)-649). The mock seeds the same managers directly ([`src/api/moonraker_client_mock.cpp:921`](../../../src/api/moonraker_client_mock.cpp#L921), `:906`), so `--test` exercises identical code paths.
...
|-------------------|------|--------------------------------------|
| subscription builder ([`src/api/moonraker_discovery_sequence.cpp:1495`](../../../src/api/moonraker_discovery_sequence.cpp#L1495)) | `zoffset::required_status_objects(hw)` | `if (hw.has_macro("SAVE_ZMOD_DATA")) subs["save_variables"] = nullptr;` |
...
- **The `is_*_printer()` helpers read the saved `printer.type` string, not live discovery** (`is_voron_printer()`, `is_creality_k1()`, `is_pfa_printer()`, ... at [`include/printer_detector.h:505`](../../../include/printer_detector.h#L505)+). They exist for rendering picks (toolhead style) and are timing-unsafe if treated as live capability answers early in boot — a fresh install has not written `printer.type` yet, and the CFS backend's K1-vs-K2 dialect check relies on Config being populated from a prior run (#1053 documented why that migration is a won't-do).
...
7. [`src/application/application.cpp:3211`](../../../src/application/application.cpp#L3211) — the self-guard comment, then the `auto_detect_and_save()` call at `:3171`; scroll up to `:2970` for `set_hardware()` on the same path.
...
11. [`src/ui/panel_widget_manager.cpp:1302`](../../../src/ui/panel_widget_manager.cpp#L1302) — `setup_gate_observers()`: one observer per distinct gate subject, coalesced rebuild; cross-reference the gate check at `:128`-137.
...
14. [`src/printer/printer_state.cpp:641`](../../../src/printer/printer_state.cpp#L641)-649 — the status fan-out to all seven managers — the sensor framework's live-data path.
...
16. [`src/printer/z_offset_persistence.cpp:15`](../../../src/printer/z_offset_persistence.cpp#L15) — the `Provider` table (one row), then the schema-read rationale at `:94`; finish with the call sites: [`src/api/moonraker_discovery_sequence.cpp:1398`](../../../src/api/moonraker_discovery_sequence.cpp#L1398), [`src/printer/printer_motion_state.cpp:206`](../../../src/printer/printer_motion_state.cpp#L206), [`src/application/application.cpp:3070`](../../../src/application/application.cpp#L3070).
