...
Because detection only runs when `printer.type` is empty — `auto_detect_and_save()` self-guards ([`src/application/application.cpp:3211`](../../../src/application/application.cpp#L3211)) — a user's manual pick is never silently overwritten on reconnect. And a win is not automatically a save: `auto_detect_and_save()` persists nothing below `AUTOSAVE_MIN_CONFIDENCE` (85, [`include/printer_detector.h:591`](../../../include/printer_detector.h#L591)), and nothing that fails to lead the runner-up by `DETECT_MIN_MARGIN` ([`include/printer_detector.h:616`](../../../include/printer_detector.h#L616)). That lead is measured on the score *before* the 100 ceiling, which otherwise flattens a real gap between two strong candidates into an apparent tie. The runtime path used to write whatever scored above zero, and since a non-empty saved type short-circuits detection forever, a single weak guess became permanent — a custom rig whose only FlashForge-ish signal was an LED named `chamber_light` scored 55 and shipped as a FlashForge Adventurer 5M Pro. Below the bar the decline is logged and the type is left empty for the user to choose, so a later reconnect with a fuller discovery snapshot gets another chance instead of being locked out. In the other direction, when a saved type exists and a fresh detection lands on a *different* model, clears `MISMATCH_MIN_CONFIDENCE` ([`include/printer_detector.h:598`](../../../include/printer_detector.h#L598)) and leads its nearest rival by the same margin, a one-time prompt offers Re-identify (launches the wizard's identify step) or Keep current — declining persists a flag and is final for that saved type, because a heavily modified printer can legitimately outvote a heuristic. Contradicting a type the user chose is a stronger claim than filling an empty one, so that bar is the auto-save bar rather than a second constant free to drift below it.
