...
Chapter 04 ended the wire story at `MoonrakerDiscoverySequence`; this chapter starts at its output. The sequence classifies `printer.objects.list` into `PrinterDiscovery` ([`include/printer_discovery.h#PrinterDiscovery`](../../../include/printer_discovery.h#L40)) — a by-value snapshot holding heaters, sensors, fans, LEDs, steppers, MCUs, kinematics, build volume, macro names, and AMS/filament-system objects — guarded by a mutex and handed out as a copy via `hardware()` ([`include/moonraker_discovery_sequence.h#hardware`](../../../include/moonraker_discovery_sequence.h#L104)). Two consumers meet it during startup: `PrinterState::set_hardware()` ([`src/application/application.cpp#setup_discovery_callbacks`](../../../src/application/application.cpp#L2993)) pushes the snapshot into state, and `PrinterDetector::auto_detect_and_save()` ([`src/application/application.cpp#setup_discovery_callbacks`](../../../src/application/application.cpp#L3174)) tries to name the printer.
...
Home-panel widgets gate dynamically, because which widgets exist is itself data. Each `PanelWidgetDef` may name a `hardware_gate_subject` ([`include/panel_widget_registry.h#hardware_gate_subject`](../../../include/panel_widget_registry.h#L25)), and `PanelWidgetManager::setup_gate_observers()` ([`src/ui/panel_widget_manager.cpp#setup_gate_observers`](../../../src/ui/panel_widget_manager.cpp#L1026)) observes every distinct gate subject with `observe_int_sync`, coalescing flips into one queued rebuild per tick — so when AMS hardware is discovered mid-session, the AMS widget appears without a restart. Two details make the UX stable rather than twitchy: gate state is part of the widget-instance key, so a gated-to-ungated transition is detected as a *change* and forces the rebuild ([`src/ui/panel_widget_manager.cpp#populate_widgets`](../../../src/ui/panel_widget_manager.cpp#L223)), and placeholder cards for hardware-gated widgets are laid out from the first frame so the grid does not jump when gates fire during discovery (`:708`). Gated widgets are placed but dimmed with a reason hint (`hardware_gate_hint`, e.g. "Requires AMS or MMU hardware") rather than removed. Chapter 09 owns the full home-widget story; the point here is the direction of the dependency: widget definitions name a *capability subject*, never a printer model or vendor.
...
One honesty note the older documentation obscures: `SensorRegistry` ([`include/sensor_registry.h#SensorRegistry`](../../../include/sensor_registry.h#L60)) — the "central registry" that fans `discover_all()`/`update_all_from_status()` across managers — is never instantiated in production; its only constructions are its own unit tests ([`tests/unit/test_sensor_registry.cpp`](../../../tests/unit/test_sensor_registry.cpp), 15 of them). The live wiring is direct: `PrinterDiscovery::parse_objects()` calls five managers' `discover()` itself ([`src/printer/printer_discovery.cpp#fsm`](../../../src/printer/printer_discovery.cpp#L105)-131), the discovery sequence's `configfile.config` step queues `ProbeSensorManager` and `AccelSensorManager` `discover_from_config()` to the main thread ([`src/api/moonraker_discovery_sequence.cpp#continue_discovery_objects`](../../../src/api/moonraker_discovery_sequence.cpp#L883)-775 — accel sensors appear *only* there), and status frames fan out to all seven from `PrinterState::update_from_status()` ([`src/printer/printer_state.cpp#update_from_status`](../../../src/printer/printer_state.cpp#L638)-646). The mock seeds the same managers directly ([`src/api/moonraker_client_mock.cpp#populate_capabilities`](../../../src/api/moonraker_client_mock.cpp#L787), `:906`), so `--test` exercises identical code paths.
...
|-------------------|------|--------------------------------------|
| subscription builder ([`src/api/moonraker_discovery_sequence.cpp#build_subscription_objects`](../../../src/api/moonraker_discovery_sequence.cpp#L1491)) | `zoffset::required_status_objects(hw)` | `if (hw.has_macro("SAVE_ZMOD_DATA")) subs["save_variables"] = nullptr;` |
...
- **The `is_*_printer()` helpers read the saved `printer.type` string, not live discovery** (`is_voron_printer()`, `is_creality_k1()`, `is_pfa_printer()`, ... at [`include/printer_detector.h#is_voron_printer`](../../../include/printer_detector.h#L494)+). They exist for rendering picks (toolhead style) and are timing-unsafe if treated as live capability answers early in boot — a fresh install has not written `printer.type` yet, and the CFS backend's K1-vs-K2 dialect check relies on Config being populated from a prior run (#1053 documented why that migration is a won't-do).
...
- **Prefer `auto_detect(discovery)` over hand-building fingerprints.** `detect()` takes a `PrinterHardwareData` you would have to assemble yourself; the wrapper ([`include/printer_detector.h#LoadStatus`](../../../include/printer_detector.h#L410)) does the conversion and is the only path production uses.
...
2. [`include/printer_discovery.h#PrinterDiscovery`](../../../include/printer_discovery.h#L40) — the class doc listing everything one snapshot holds; skim `parse_objects()` at `:52` far enough to see the chamber-heater scoring lambdas (keyword tier plus object-type tiebreak).
3. [`include/printer_detector.h#PrinterDetector`](../../../include/printer_detector.h#L121) — the static class; read `detect()`'s contract at `:111`, then `apply_preset_with_variants()` at `:192` for the firmware-variant trick, and `LoadStatus` at `:375`.
...
6. [`src/printer/printer_detector.cpp#load`](../../../src/printer/printer_detector.cpp#L75) — the two-phase load: `find_readable("printer_database.json")`, then the `printer_database.d` merge at `:100`-161.
7. [`src/application/application.cpp#setup_discovery_callbacks`](../../../src/application/application.cpp#L3162) — the self-guard comment, then the `auto_detect_and_save()` call at `:3171`; scroll up to `:2970` for `set_hardware()` on the same path.
8. [`include/printer_capabilities_state.h#PrinterCapabilitiesState`](../../../include/printer_capabilities_state.h#L354) — all 22 subjects in one block; then `set_hardware()`'s contract at `:59` and the `AsyncLifetimeGuard` comment at `:328`.
9. [`include/capability_overrides.h#OverrideState`](../../../include/capability_overrides.h#L19) — `OverrideState` three-state logic and the seven `capability::` constants at `:31`.
10. [`ui_xml/motion_panel.xml#btn_qgl`](../../../ui_xml/motion_panel.xml#L172) — capability gating in XML: `bind_flag_if_eq` on `printer_has_qgl` (and `printer_has_z_tilt` at `:149`).
11. [`src/ui/panel_widget_manager.cpp#setup_gate_observers`](../../../src/ui/panel_widget_manager.cpp#L1026) — `setup_gate_observers()`: one observer per distinct gate subject, coalesced rebuild; cross-reference the gate check at `:128`-137.
12. [`include/sensor_registry.h#ISensorManager`](../../../include/sensor_registry.h#L22) — `ISensorManager`'s three discovery arms with their source comments; `SensorRegistry` at `:60`, knowing production never constructs it.
...
14. [`src/printer/printer_state.cpp#update_from_status`](../../../src/printer/printer_state.cpp#L638)-646 — the status fan-out to all seven managers — the sensor framework's live-data path.
...
16. [`src/printer/z_offset_persistence.cpp#Provider`](../../../src/printer/z_offset_persistence.cpp#L15) — the `Provider` table (one row), then the schema-read rationale at `:94`; finish with the call sites: [`src/api/moonraker_discovery_sequence.cpp#build_subscription_objects`](../../../src/api/moonraker_discovery_sequence.cpp#L1394), [`src/printer/printer_motion_state.cpp#update_from_status`](../../../src/printer/printer_motion_state.cpp#L206), [`src/application/application.cpp#setup_discovery_callbacks`](../../../src/application/application.cpp#L3021).
...
18. [`include/z_offset_utils.h#helix::zoffset`](../../../include/z_offset_utils.h#L18) — the enum's consumers: `is_auto_saved()` at `:18`, then `apply_and_save()` at `:76` taking the API and the strategy — generic calibration code that never learns which printer it is talking to.
