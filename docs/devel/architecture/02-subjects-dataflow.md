...
The dequeue happens on the main thread. `Application::main_loop()` ([`src/application/application.cpp:4030`](../../../src/application/application.cpp#L4030), entered once from `run()`) iterates: heartbeat and input housekeeping, `check_timeouts()`, then `process_notifications()` ([`src/application/application.cpp:4175`](../../../src/application/application.cpp#L4175)), and only then `lv_timer_handler()` (`:4136`) where rendering happens. `MoonrakerManager::process_notifications()` ([`src/application/moonraker_manager.cpp:252`](../../../src/application/moonraker_manager.cpp#L252)) drains the whole queue each pass: `_connection_state` markers go to `PrinterState::set_printer_connection_state()`, while `notify_status_update` frames are unpacked — Klipper `eventtime` and the cached-snapshot marker are read out alongside the status object — and handed to the state model:
...
A second, smaller intake exists: RPC replies and webhooks events (Klippy ready/shutdown notifications, `printer.info` seeds) arrive as callbacks on the WebSocket thread and go through the public setters, which marshal themselves. `PrinterState::set_klippy_state()` ([`src/printer/printer_state.cpp:669`](../../../src/printer/printer_state.cpp#L669)) wraps `helix::async::call_method(this, &PrinterState::set_klippy_state_internal, state)` — a `queue_update()` wrapper from [`include/async_helpers.h`](../../../include/async_helpers.h) — while `set_printer_connection_state()` (`:651`) uses `async_lifetime_.defer()`, the same deferral with a generation guard so callbacks queued after teardown are dropped. AMS data does not flow through `PrinterState` at all: `AmsState` is fed by its own backend events (`sync_from_backend()`, [`include/ams_state.h:1379`](../../../include/ams_state.h#L1379)).
...
- **Equal-value sets do not notify.** `lv_subject_set_int()` notifies only on change (`lib/lvgl/src/core/lv_observer.c:139`); if observers must re-run on an unchanged value, call `lv_subject_notify()` explicitly (the bed-temp pattern, [`printer_temperature_state.cpp:373`](../../../src/printer/printer_temperature_state.cpp#L373)).
...
2. [`src/application/application.cpp:4172`](../../../src/application/application.cpp#L4172) — the main-loop strip from `check_timeouts()` through `process_notifications()` (`:4107`) to `lv_timer_handler()` (`:4136`). This is the ordering the mermaid diagram encodes.
