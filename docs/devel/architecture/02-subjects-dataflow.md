...
The dequeue happens on the main thread. `Application::main_loop()` ([`src/application/application.cpp:4029`](../../../src/application/application.cpp#L4029), entered once from `run()`) iterates: heartbeat and input housekeeping, `check_timeouts()`, then `process_notifications()` ([`src/application/application.cpp:4174`](../../../src/application/application.cpp#L4174)), and only then `lv_timer_handler()` (`:4136`) where rendering happens. `MoonrakerManager::process_notifications()` ([`src/application/moonraker_manager.cpp:252`](../../../src/application/moonraker_manager.cpp#L252)) drains the whole queue each pass: `_connection_state` markers go to `PrinterState::set_printer_connection_state()`, while `notify_status_update` frames are unpacked — Klipper `eventtime` and the cached-snapshot marker are read out alongside the status object — and handed to the state model:
...
2. [`src/application/application.cpp:4171`](../../../src/application/application.cpp#L4171) — the main-loop strip from `check_timeouts()` through `process_notifications()` (`:4107`) to `lv_timer_handler()` (`:4136`). This is the ordering the mermaid diagram encodes.
3. [`src/printer/printer_state.cpp:386`](../../../src/printer/printer_state.cpp#L386) — `update_from_status()`: the `state_mutex_` lock and the fan-out to state components. Then `:651` and `:659` for the two marshalling flavors (`async_lifetime_.defer` vs `helix::async::call_method`), and `:711` `set_klippy_state_internal()` — the chokepoint that also resets Klippy-volatile subjects.
