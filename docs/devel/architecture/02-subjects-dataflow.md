...
The dequeue happens on the main thread. `Application::main_loop()` ([`src/application/application.cpp:4081`](../../../src/application/application.cpp#L4081), entered once from `run()`) iterates: heartbeat and input housekeeping, `check_timeouts()`, then `process_notifications()` ([`src/application/application.cpp:4226`](../../../src/application/application.cpp#L4226)), and only then `lv_timer_handler()` (`:4136`) where rendering happens. `MoonrakerManager::process_notifications()` ([`src/application/moonraker_manager.cpp:254`](../../../src/application/moonraker_manager.cpp#L254)) drains the whole queue each pass: `_connection_state` markers go to `PrinterState::set_printer_connection_state()`, while `notify_status_update` frames are unpacked — Klipper `eventtime` and the cached-snapshot marker are read out alongside the status object — and handed to the state model:
...
2. [`src/application/application.cpp:4223`](../../../src/application/application.cpp#L4223) — the main-loop strip from `check_timeouts()` through `process_notifications()` (`:4107`) to `lv_timer_handler()` (`:4136`). This is the ordering the mermaid diagram encodes.
3. [`src/printer/printer_state.cpp:389`](../../../src/printer/printer_state.cpp#L389) — `update_from_status()`: the `state_mutex_` lock and the fan-out to state components. Then `:651` and `:659` for the two marshalling flavors (`async_lifetime_.defer` vs `helix::async::call_method`), and `:711` `set_klippy_state_internal()` — the chokepoint that also resets Klippy-volatile subjects.
...
9. [`src/printer/printer_state.cpp:300`](../../../src/printer/printer_state.cpp#L300) — the tail of `init_subjects()`: macro use, then the `StaticSubjectRegistry::register_deinit` self-registration at `:324`.
