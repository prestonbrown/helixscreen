...
Which spool is mounted where is *not* AMS state. `sync_from_backend()` bridges for every slot with a `mapped_tool` ([`src/printer/ams_state.cpp:1752`](../../../src/printer/ams_state.cpp#L1752)-1769): a slot with `spoolman_id > 0` calls `ToolState::assign_spool()`.
...
One trap the interface answers: pushing "active spool" to Spoolman is gated on `manages_active_spool()` ([`src/printer/ams_state.cpp:2922`](../../../src/printer/ams_state.cpp#L2922)-2930). AFC, for instance, updates Spoolman itself when HelixScreen sends its native spool command — calling Spoolman directly would update the widget while bypassing the firmware's own state (#644).
...
- **Clearing a spool assignment is conditional.** Forward-clear only when the backend persists spools in firmware; tool-changer-style backends get the *reverse* sync instead ([`ams_state.cpp:1761`](../../../src/printer/ams_state.cpp#L1761)-1775). Clearing unconditionally destroys the just-loaded assignment.
...
- **Event names are plain strings — a typo compiles.** `EVENT_*` are `constexpr const char*` and `on_backend_event()` is an if/else chain ([`ams_state.cpp:2099`](../../../src/printer/ams_state.cpp#L2099)-2123); a misspelled name falls through every branch with nothing but the entry trace log. If a new event does nothing, check the spelling first.
- **`clear_backends()` is a wider reset than it looks.** It unregisters the per-slot `FilamentConsumptionTracker` sinks *before* stopping backends (they flush on the way out, [`ams_state.cpp:849`](../../../src/printer/ams_state.cpp#L849)-858), resets runout edge state, and drops ToolState's AMS topology (`:880`) so stale tool pills vanish between backend disappearance and the next reconnect.
...
10. [`src/printer/ams_state.cpp:1631`](../../../src/printer/ams_state.cpp#L1631) — the ToolState bridge: forward assign, the firmware-persistence-gated clear, and the reverse sync for tool-changer backends below it.
