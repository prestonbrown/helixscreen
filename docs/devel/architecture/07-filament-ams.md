...
8. [`src/printer/ams_state.cpp:2141`](../../../src/printer/ams_state.cpp#L2141) — `on_backend_event()`: queue-only body, shutdown-flag guard, the SLOT_CHANGED parse-or-full-sync fallback; then follow one queued call into `sync_backend()` at `:1284`.
9. [`include/ams_state.h:1551`](../../../include/ams_state.h#L1551) — `BackendSlotSubjects` and its lifetime-token comment; glance at the storage members at `:1520`-1522 (mutex, `backends_`, `secondary_slot_subjects_`).
