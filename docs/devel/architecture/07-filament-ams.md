...
| `AmsBackendCfs` | [`include/ams_backend_cfs.h:120`](../../../include/ams_backend_cfs.h#L120) | Creality Filament System (K2, RS-485 boxes) |
...
`AmsType` ([`include/ams_types.h:42`](../../../include/ams_types.h#L42)) enumerates them plus `NONE`. The single place an `AmsType` becomes a class is `AmsBackend::create(AmsType, api, client)` ([`src/printer/ams_backend.cpp:542`](../../../src/printer/ams_backend.cpp#L542)); mock mode (`RuntimeConfig::should_mock_ams()`) short-circuits to `AmsBackendMock`. This is the vendor rule from chapter 06 working as dispatch: `AmsState`'s header contains vendor names *only in comments*, and a ninth system means one new subclass plus one enum value, factory case, and detection entry — no edits to generic code.

Detection feeding that factory is a priority ladder in `PrinterDiscovery::parse_objects()` ([`include/printer_discovery.h:536`](../../../include/printer_discovery.h#L536)-566): a real MMU (Happy Hare, AFC, AD5X IFS, CFS, ACE, QIDI Box) always wins, then native Snapmaker hardware, then a standalone tool changer. `parse_objects()` ends by calling `AmsState::instance().init_backend_from_hardware()` directly ([`src/printer/printer_discovery.cpp:101`](../../../src/printer/printer_discovery.cpp#L101)); `init_backends_from_hardware()` ([`src/printer/ams_state.cpp:709`](../../../src/printer/ams_state.cpp#L709)) skips mock mode, skips if backends already exist, creates and `start()`s one backend per detected system, then syncs immediately so the `ams_slot_count` gate lights up without waiting for the first async event (`:762`).
...
Backends emit events from background threads (their Moonraker subscriptions fire on libhv). The whole safety story is that `on_backend_event()` ([`src/printer/ams_state.cpp:2065`](../../../src/printer/ams_state.cpp#L2065)) touches neither the mutex nor a subject: it only posts a `helix::ui::queue_update()` lambda whose body — already on the main thread — checks `s_shutdown_flag`, then calls `sync_backend(index)` or `update_slot_for_backend(index, slot)`, which take the recursive mutex and write subjects (`:1273`, `:1311`). The mutex still matters because background threads call thread-safe `AmsState` queries (`get_backend()`, `backend_count()`, ...) concurrently with main-thread syncs. After every sync the queued body bumps `ams_data_revision_` so code waiting for backend data to land can re-read (`:2088`).
...
One trap the interface answers: pushing "active spool" to Spoolman is gated on `manages_active_spool()` ([`src/printer/ams_state.cpp:2647`](../../../src/printer/ams_state.cpp#L2647)-2658). AFC, for instance, updates Spoolman itself when HelixScreen sends its native spool command — calling Spoolman directly would update the widget while bypassing the firmware's own state (#644).
...
- **Do not write subjects from backend-event context.** The event path queues *before* touching anything ([`ams_state.cpp:2071`](../../../src/printer/ams_state.cpp#L2071)); the queued body is where mutex + subjects happen. A shortcut around `queue_update` reintroduces the bg-thread LVGL crash family (chapter 03).
...
- **Clearing a spool assignment is conditional.** Forward-clear only when the backend persists spools in firmware; tool-changer-style backends get the *reverse* sync instead ([`ams_state.cpp:1644`](../../../src/printer/ams_state.cpp#L1644)-1651). Clearing unconditionally destroys the just-loaded assignment.
...
- **Event names are plain strings — a typo compiles.** `EVENT_*` are `constexpr const char*` and `on_backend_event()` is an if/else chain ([`ams_state.cpp:2093`](../../../src/printer/ams_state.cpp#L2093)-2123); a misspelled name falls through every branch with nothing but the entry trace log. If a new event does nothing, check the spelling first.
- **`clear_backends()` is a wider reset than it looks.** It unregisters the per-slot `FilamentConsumptionTracker` sinks *before* stopping backends (they flush on the way out, [`ams_state.cpp:829`](../../../src/printer/ams_state.cpp#L829)-838), resets runout edge state, and drops ToolState's AMS topology (`:857`) so stale tool pills vanish between backend disappearance and the next reconnect.
- **Some XML subject names are not the member names.** Most subjects register under their member spelling (`tool_map_version`), but a few manual registrations add a prefix — member `filament_loaded_` binds as `ams_filament_loaded` ([`ams_state.cpp:255`](../../../src/printer/ams_state.cpp#L255)-263). When an XML binding reports "No subject was found", check the `lv_xml_register_subject` call, not the header.
...
8. [`src/printer/ams_state.cpp:2065`](../../../src/printer/ams_state.cpp#L2065) — `on_backend_event()`: queue-only body, shutdown-flag guard, the SLOT_CHANGED parse-or-full-sync fallback; then follow one queued call into `sync_backend()` at `:1273`.
9. [`include/ams_state.h:1479`](../../../include/ams_state.h#L1479) — `BackendSlotSubjects` and its lifetime-token comment; glance at the storage members at `:1493`-1495 (mutex, `backends_`, `secondary_slot_subjects_`).
...
13. [`include/ams_state.h:617`](../../../include/ams_state.h#L617) — the two-scope lifetime doc (`get_subjects_lifetime()` vs the per-slot tokens) with its PrintStatusPanel example; the best single comment on when observers need a token.
