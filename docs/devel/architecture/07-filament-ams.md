...
Backends emit events from background threads (their Moonraker subscriptions fire on libhv). The whole safety story is that `on_backend_event()` ([`src/printer/ams_state.cpp#on_backend_event`](../../../src/printer/ams_state.cpp#L2421)) touches neither the mutex nor a subject: it only posts a `helix::ui::queue_update()` lambda whose body — already on the main thread — checks `s_shutdown_flag`, then calls `sync_backend(index)` or `update_slot_for_backend(index, slot)`, which take the recursive mutex and write subjects (`:1284`, `:1322`). The mutex still matters because background threads call thread-safe `AmsState` queries (`get_backend()`, `backend_count()`, ...) concurrently with main-thread syncs. After every sync the queued body bumps `ams_data_revision_` so code waiting for backend data to land can re-read (`:2257`).

Routing is by captured index: `add_backend()` ([`src/printer/ams_state.cpp#set_backend`](../../../src/printer/ams_state.cpp#L974)) registers a lambda that closes over the backend's position in `backends_` (`:784`), so events from concurrent systems cannot cross wires.
...
- **Backend 0** writes the flat arrays every single-backend XML binding already knows — `slot_colors_[i]`, `slot_statuses_[i]`, `slot_fills_[i]`, plus string and live-state families — inside `sync_from_backend()` ([`src/printer/ams_state.cpp#sync_from_backend`](../../../src/printer/ams_state.cpp#L1647), per-slot loop at `:1698`).
...
`ToolState::assign_spool()` ([`src/printer/tool_state.cpp#same_displayed_weight`](../../../src/printer/tool_state.cpp#L787)) splits the record in two. Identity (spool id + name) is the durable half: a change sets `spool_dirty_` and logs at info. Weights are a cache — firmware reports them as continuous floats, and the code comment preserves the war story (`:641`): an exact compare on bundle L53W5PKG meant 590 rewrites of the JSON, 590 Moonraker DB POSTs and 590 panel rebuilds in one session. `same_displayed_weight()` (`:617`) compares at whole grams — `std::lround(a) == std::lround(b)`, against the last *stored* value so a slow slide fires once per gram — and a weight-only change bumps `tools_version_` for UI refresh while never marking the record dirty.
...
2. [`include/ams_backend.h`](../../../include/ams_backend.h) — the interface: the event constants at `:72`-80, then the capability defaults `manages_active_spool()` at `:191`, `tracks_weight_locally()` at `:205`, and `has_firmware_spool_persistence()` at `:2004`; finish with the factory declarations at `:2210`-2239.
...
6. [`src/printer/ams_state.cpp#init_backends_from_hardware`](../../../src/printer/ams_state.cpp#L909) — `init_backends_from_hardware()`: mock skip, double-init guard, the create-start loop, the immediate sync at `:769`.
7. [`src/printer/ams_state.cpp#set_backend`](../../../src/printer/ams_state.cpp#L974) — `add_backend()`: the captured-index event lambda at `:784`, secondary-subject allocation at `:797`, consumption-sink registration at `:802`.
8. [`src/printer/ams_state.cpp#on_backend_event`](../../../src/printer/ams_state.cpp#L2421) — `on_backend_event()`: queue-only body, shutdown-flag guard, the SLOT_CHANGED parse-or-full-sync fallback; then follow one queued call into `sync_backend()` at `:1284`.
9. [`include/ams_state.h#AmsState`](../../../include/ams_state.h#L1688) — `BackendSlotSubjects` and its lifetime-token comment; glance at the storage members at `:1558`-1560 (mutex, `backends_`, `secondary_slot_subjects_`).
...
11. [`src/printer/tool_state.cpp#helix`](../../../src/printer/tool_state.cpp#L780) — `same_displayed_weight()` (the whole-gram compare) and the L53W5PKG comment at `:641`; then `assign_spool()` at `:624` for the identity/weight split, and the save path at `:704` (atomic write, symlink resolution) and `:801` (local-first, DB fire-and-forget).
