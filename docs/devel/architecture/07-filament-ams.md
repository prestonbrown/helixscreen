...
Backends emit events from background threads (their Moonraker subscriptions fire on libhv). The whole safety story is that `on_backend_event()` ([`src/printer/ams_state.cpp:2141`](../../../src/printer/ams_state.cpp#L2141)) touches neither the mutex nor a subject: it only posts a `helix::ui::queue_update()` lambda whose body — already on the main thread — checks `s_shutdown_flag`, then calls `sync_backend(index)` or `update_slot_for_backend(index, slot)`, which take the recursive mutex and write subjects (`:1284`, `:1322`). The mutex still matters because background threads call thread-safe `AmsState` queries (`get_backend()`, `backend_count()`, ...) concurrently with main-thread syncs. After every sync the queued body bumps `ams_data_revision_` so code waiting for backend data to land can re-read (`:2109`).
...
- **Backend 0** writes the flat arrays every single-backend XML binding already knows — `slot_colors_[i]`, `slot_statuses_[i]`, `slot_fills_[i]`, plus string and live-state families — inside `sync_from_backend()` ([`src/printer/ams_state.cpp:1404`](../../../src/printer/ams_state.cpp#L1404), per-slot loop at `:1698`).
- **Backends at index 1+** get a `BackendSlotSubjects` struct ([`include/ams_state.h:1551`](../../../include/ams_state.h#L1551)) allocated at `add_backend()` time — dynamic `colors`/`statuses`/`fills` vectors sized to the backend's slot count. These subjects are destroyed on backend rediscovery, so the struct carries a `SubjectLifetime` token and the token-taking accessor overloads (`get_slot_color_subject(backend, slot, lifetime)` at `:1007`) hand it out; an observer that skips the token is chapter 03 bug #705 waiting.
...
Which spool is mounted where is *not* AMS state. `sync_from_backend()` bridges for every slot with a `mapped_tool` ([`src/printer/ams_state.cpp:1794`](../../../src/printer/ams_state.cpp#L1794)-1811): a slot with `spoolman_id > 0` calls `ToolState::assign_spool()`.
...
One trap the interface answers: pushing "active spool" to Spoolman is gated on `manages_active_spool()` ([`src/printer/ams_state.cpp:2964`](../../../src/printer/ams_state.cpp#L2964)-2972). AFC, for instance, updates Spoolman itself when HelixScreen sends its native spool command — calling Spoolman directly would update the widget while bypassing the firmware's own state (#644).
...
- **Do not write subjects from backend-event context.** The event path queues *before* touching anything ([`ams_state.cpp:2113`](../../../src/printer/ams_state.cpp#L2113)); the queued body is where mutex + subjects happen. A shortcut around `queue_update` reintroduces the bg-thread LVGL crash family (chapter 03).
...
- **Clearing a spool assignment is conditional.** Forward-clear only when the backend persists spools in firmware; tool-changer-style backends get the *reverse* sync instead ([`ams_state.cpp:1803`](../../../src/printer/ams_state.cpp#L1803)-1817). Clearing unconditionally destroys the just-loaded assignment.
...
- **Event names are plain strings — a typo compiles.** `EVENT_*` are `constexpr const char*` and `on_backend_event()` is an if/else chain ([`ams_state.cpp:2141`](../../../src/printer/ams_state.cpp#L2141)-2165); a misspelled name falls through every branch with nothing but the entry trace log. If a new event does nothing, check the spelling first.
...
8. [`src/printer/ams_state.cpp:2141`](../../../src/printer/ams_state.cpp#L2141) — `on_backend_event()`: queue-only body, shutdown-flag guard, the SLOT_CHANGED parse-or-full-sync fallback; then follow one queued call into `sync_backend()` at `:1331`.
9. [`include/ams_state.h:1552`](../../../include/ams_state.h#L1552) — `BackendSlotSubjects` and its lifetime-token comment; glance at the storage members at `:1566`-1568 (mutex, `backends_`, `secondary_slot_subjects_`).
10. [`src/printer/ams_state.cpp:1794`](../../../src/printer/ams_state.cpp#L1794) — the ToolState bridge: forward assign, the firmware-persistence-gated clear, and the reverse sync for tool-changer backends below it.
