...
- **Backend 0** writes the flat arrays every single-backend XML binding already knows — `slot_colors_[i]`, `slot_statuses_[i]`, `slot_fills_[i]`, plus string and live-state families — inside `sync_from_backend()` ([`src/printer/ams_state.cpp:1398`](../../../src/printer/ams_state.cpp#L1398), per-slot loop at `:1698`).
...
9. [`include/ams_state.h:1543`](../../../include/ams_state.h#L1543) — `BackendSlotSubjects` and its lifetime-token comment; glance at the storage members at `:1558`-1560 (mutex, `backends_`, `secondary_slot_subjects_`).
