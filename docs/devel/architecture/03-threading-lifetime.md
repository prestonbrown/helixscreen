...
(verbatim from [`src/printer/printer_state.cpp#set_printer_connection_state`](../../../src/printer/printer_state.cpp#L670)). `set_klippy_state()` (`:659`) uses the `helix::async::call_method()` flavor — same queue, less ceremony. Chapter 02 dissects both.
...
4. [`src/printer/printer_state.cpp#set_printer_connection_state`](../../../src/printer/printer_state.cpp#L670) — `set_printer_connection_state()` (defer through the guard), then `:659` `set_klippy_state()` (`call_method` flavor). The whole marshalling-setter pattern in twenty lines.
