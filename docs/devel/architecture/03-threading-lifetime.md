...
There is exactly one thread that may call `lv_*` anything: the main thread, which enters `Application::main_loop()` ([`src/application/application.cpp:4071`](../../../src/application/application.cpp#L4071)) and never leaves it until shutdown. Everything else is background:
...
(verbatim from [`src/printer/printer_state.cpp:654`](../../../src/printer/printer_state.cpp#L654)). `set_klippy_state()` (`:659`) uses the `helix::async::call_method()` flavor — same queue, less ceremony. Chapter 02 dissects both.
...
4. [`src/printer/printer_state.cpp:654`](../../../src/printer/printer_state.cpp#L654) — `set_printer_connection_state()` (defer through the guard), then `:659` `set_klippy_state()` (`call_method` flavor). The whole marshalling-setter pattern in twenty lines.
