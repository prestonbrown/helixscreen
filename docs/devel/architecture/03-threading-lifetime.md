...
flowchart LR
    ...
    subgraph MAIN["Main thread - owns every lv_obj and lv_subject"]
        ...
        TH["lv_timer_handler<br/>UpdateQueue drain timer,<br/>LVGL timers, render"]
        LOOP --> TH
    end
...
There is exactly one thread that may call `lv_*` anything: the main thread, which enters `Application::main_loop()` ([`src/application/application.cpp#main_loop`](../../../src/application/application.cpp#L4032)) and never leaves it until shutdown. Everything else is background:
...
- **Long-lived owned workers.** `UpdateChecker` runs `worker_thread_` ([`src/system/update_checker.cpp#check_for_updates`](../../../src/system/update_checker.cpp#L2580), joined in the destructor at `:691`) and `download_thread_` (`:1341`); `TelemetryManager` spawns a per-batch `send_thread_` ([`src/system/telemetry_manager.cpp#try_send`](../../../src/system/telemetry_manager.cpp#L936), joined at `:463`); the camera stream and the XML hot-reload poller each own one. Fifty-two files under `src/` mention `std::thread` — every spawn is either joined in its owner's destructor, bounded-then-detached as described below, or wrapped in the try/catch form described next.
- **One worker is deliberately not joined: `UpdateChecker::download_thread_`.** It can be parked inside libhv's synchronous `requests::downloadFile()`, whose `req->timeout` is 3600 seconds and which exposes no abort hook — its progress callback returns `void`. Joining it on the LVGL thread froze the touchscreen, and joining it at teardown hung shutdown for the same hour. `reap_download_thread()` ([`src/system/update_checker.cpp#reap_download_thread`](../../../src/system/update_checker.cpp#L1350)) waits a bounded time for the worker to finish — 1s from `shutdown()` (`:764`), 250ms from the destructor (`:697`) — then **detaches** and lets it die with the process, the same escape hatch `HttpExecutor::stop()` takes for the same reason. `shutting_down_` is set first, so the detached worker bails at its next check (`:1486`) without touching a subject, the mutex, or the UpdateQueue.
...
The heuristic for everything else: if you are in a callback from libhv, an `HttpExecutor`, `BusThread`, or any `std::thread`, you are on a background thread, and the rest of this chapter applies. The codebase itself makes the same distinction at runtime: `main()` records the main thread's id once (`internal::set_main_thread_id()`, [`include/async_lifetime_guard.h#internal`](../../../include/async_lifetime_guard.h#L63)) and `internal::on_main_thread()` (`:68`) answers the question conservatively (it returns true during early init, before any thread exists to be wrong about) — that is the predicate behind the anti-pattern detector below.

### One bridge: the `UpdateQueue` contract

Chapter 02 covered the data-flow view — the notification queue that hands raw JSON to `process_notifications()` ([`src/application/application.cpp#main_loop`](../../../src/application/application.cpp#L4154)) before `lv_timer_handler()` (`:4136`) runs. The `UpdateQueue` is the general-purpose sibling: any thread enqueues a tagged lambda with `helix::ui::queue_update()`; an LVGL timer created in `init()` on the display refresh period ([`include/ui_update_queue.h#init`](../../../include/ui_update_queue.h#L119)) drains `process_pending()` (`:441`) on the main thread inside `lv_timer_handler()`.
...
(verbatim from [`src/printer/printer_state.cpp#set_printer_connection_state`](../../../src/printer/printer_state.cpp#L667)). `set_klippy_state()` (`:659`) uses the `helix::async::call_method()` flavor — same queue, less ceremony. Chapter 02 dissects both.
...
1. [`include/ui_update_queue.h#helix::ui`](../../../include/ui_update_queue.h#L94) — the `UpdateQueue` class: timer creation in `init()` (`:119`), the tagged drain `process_pending()` (`:441`), `ScopedFreeze` (`:249`). The comments on "why not `lv_async_call`" are the design argument in three lines.
2. [`include/async_lifetime_guard.h#expired`](../../../include/async_lifetime_guard.h#L115) — `LifetimeToken::expired()` with the background-thread detector inline; then `:165` `defer()` and its pre-enqueue generation check (the doomed-callback drop), and `:354` `bg_cb()` — read the doc comment for the short-form/long-form tradeoff.
...
4. [`src/printer/printer_state.cpp#set_printer_connection_state`](../../../src/printer/printer_state.cpp#L667) — `set_printer_connection_state()` (defer through the guard), then `:659` `set_klippy_state()` (`call_method` flavor). The whole marshalling-setter pattern in twenty lines.
5. [`include/ui_observer_guard.h#SubjectLifetime`](../../../include/ui_observer_guard.h#L40) — the `SubjectLifetime` contract in the doc comment; then `reset()` (`:123`) with its two death checks: token value (`:137`) and invalidation epoch (`:150`).
...
9. [`include/ui_utils.h#helix::ui`](../../../include/ui_utils.h#L96) — the comment explaining why `safe_delete` (`:102`) is unsafe inside queued callbacks, then the deferred family at `:200`, `:284`, `:345`.
10. [`include/ui_timer_guard.h#helix::ui`](../../../include/ui_timer_guard.h#L18) — `lv_timer_cancel_safe()`: five lines that neuter instead of unlink; then `LvglTimerGuard` (`:33`).
