...
Besides the panels, a few subsystems hold the client or API directly from startup: `PrintHistoryManager` receives both at construction ([`src/application/application.cpp:2154`](../../../src/application/application.cpp#L2154)), `PrintStartCollector` takes the client (`MoonrakerManager::init_print_start_collector()`), and `SoundManager` keeps a client pointer for M300 gcode audio ([`src/application/moonraker_manager.cpp:484`](../../../src/application/moonraker_manager.cpp#L484)). All of them receive interface types.
...
- **Teardown order is load-bearing:** `shutdown()` drops the client *first* — its destructor waits for in-flight libhv callbacks that capture raw pointers to the API ([`src/application/moonraker_manager.cpp:167`](../../../src/application/moonraker_manager.cpp#L167), #628) — then macro analysis, then the API.
...
7. [`src/application/moonraker_manager.cpp:354`](../../../src/application/moonraker_manager.cpp#L354) — `create_client()`: ESP32 arm, mock arm (`HELIX_MOCK_PRINTER` parsing, `--sim-speed`, `MockHttpFileServer`), real arm, and the mock-to-mock AMS wiring. Then `create_api()` at `:600`.
...
12. [`src/application/moonraker_manager.cpp:588`](../../../src/application/moonraker_manager.cpp#L588) — `register_callbacks()`: the one-hop `bg_cb()` event marshalling and the `_connection_state` markers joining the notification queue; cross-reference `process_notifications()` at `:234`.
13. [`include/runtime_config.h:195`](../../../include/runtime_config.h#L195) — `should_mock_moonraker()` and its siblings; scroll to `:241` for the `constexpr` false arms that make production builds mock-proof.
