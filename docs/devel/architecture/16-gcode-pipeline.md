...
- The scan-finish path writes through, but **only when authoritative** (`:1767`): a degraded finish (download failed) carries an empty set with no information, and persisting it would freeze "no tools" for the file — on 2D-only platforms no viewer parse ever repairs that.
...
9. [`include/tools_used_cache.h:20`](../../../include/tools_used_cache.h#L20) — the cache: keying, LRU bound, and the "empty set is legitimate" contract at `:24`. Then its three call sites in the detail view (`:481`, `:1411`, `:1768`).
