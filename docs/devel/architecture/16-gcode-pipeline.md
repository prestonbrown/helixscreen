...
- The settings observer would otherwise fire once at startup with the persisted value and silently overwrite the cmdline pin ~16ms after it was applied; it now stands down when the cmdline pinned a mode ([`src/ui/ui_panel_print_status.cpp:402`](../../../src/ui/ui_panel_print_status.cpp#L402)).
...
- **Stage 2 — segments, downward.** Only when footprints overlap: walk layers from `current_layer_` **downward**, first layer that produces a hit wins (`:1709`) — a tap over a stack picks what is visually on top of it. In streaming mode this walk is **cache-only**: `try_get_layer_segments()` (`:1724`) skips uncached layers rather than seeking and parsing, because a hit-test must never freeze the UI on a 2-core board.

The gestures on top of it ([`src/ui/ui_gcode_viewer.cpp`](../../../src/ui/ui_gcode_viewer.cpp)): a **press-and-hold of 1000ms** (`LONG_PRESS_THRESHOLD_MS`, `:825` — deliberately double the app-wide 500ms gesture timeout, because the consequence is canceling an object's print) picks the object under the initial press and fires the long-press callback into the exclusion flow; a **tap** (release with minimal movement, no pinch, long-press not already fired) toggles single-selection (`:1083`), which draws the corner-bracket wireframe via `render_selection_brackets()` ([`src/rendering/gcode_layer_renderer.cpp#render_selection_brackets`](../../../src/rendering/gcode_layer_renderer.cpp#L1822)). The **skip-objects button** (`btn_objects`, [`ui_xml/components/print_status_preview_card.xml#btn_objects`](../../../ui_xml/components/print_status_preview_card.xml#L38), visible when the `exclude_objects_available` subject is set) toggles the map + side-list panel (`on_objects_clicked`, [`src/ui/ui_panel_print_status.cpp#on_objects_clicked`](../../../src/ui/ui_panel_print_status.cpp#L2107)).
...
- **Streaming and full-file are mutually exclusive by construction** — each setter nulls the other's pointer ([`src/rendering/gcode_layer_renderer.cpp#set_gcode`](../../../src/rendering/gcode_layer_renderer.cpp#L117) vs `:150`). Never hand a renderer both; the second call silently wins.
...
1. [`include/gcode_parser.h#ToolpathSegment`](../../../include/gcode_parser.h#L222) — `ToolpathSegment`, `Layer`, `GCodeObject`, then `ParsedGCodeFile` at `:291`: the interned object-name design (`object_name_table`) that picking compares with `int16_t`s.
...
3. [`include/gcode_streaming_config.h#helix`](../../../include/gcode_streaming_config.h#L11) — the three-step config hierarchy; then `should_use_gcode_streaming()` at `:71`.
...
6. [`src/ui/ui_print_select_detail_view.cpp#local_gcode_source`](../../../src/ui/ui_print_select_detail_view.cpp#L574) — `local_gcode_source()`: the same-host probe and size staleness check; then `reclaim_download()` at `:627` for the delete gate.
7. [`include/gcode_footer_summary.h#GcodeFooterSummary`](../../../include/gcode_footer_summary.h#L23) — the whole footer contract in one header: `GcodeFooterSummary`, the exact-key rules at `:62`, and the window bounds at `:88`.
...
9. [`include/tools_used_cache.h#ToolsUsedCache`](../../../include/tools_used_cache.h#L20) — the cache: keying, LRU bound, and the "empty set is legitimate" contract at `:24`. Then its three call sites in the detail view ([`src/ui/ui_print_select_detail_view.cpp#show`](../../../src/ui/ui_print_select_detail_view.cpp#L463), [`src/ui/ui_print_select_detail_view.cpp#try_extract_gcode_colors`](../../../src/ui/ui_print_select_detail_view.cpp#L1258), [`src/ui/ui_print_select_detail_view.cpp#apply_scan_result`](../../../src/ui/ui_print_select_detail_view.cpp#L1647)).
...
11. [`src/ui/ui_panel_print_status.cpp#on_activate`](../../../src/ui/ui_panel_print_status.cpp#L1146) — the precedence chain applied; then the pinned-mode guard at `:346`.
12. [`src/rendering/gcode_layer_renderer.cpp#pick_object_at`](../../../src/rendering/gcode_layer_renderer.cpp#L1585) — `pick_object_at()` in full: stage-1 clamping and skips (`:1576-1707`), the single-candidate return (`:1679`), the downward walk (`:1709`), and the cache-only streaming branch (`:1723`).
13. [`src/ui/ui_gcode_viewer.cpp#gcode_viewer_draw_cb`](../../../src/ui/ui_gcode_viewer.cpp#L882) — the long-press constant and its comment; then the release callback's tap handling at `:1083`.
