...
Before anything downloads, `start_tail_summary_scan()` ([`src/ui/ui_print_select_detail_view.cpp:1709`](../../../src/ui/ui_print_select_detail_view.cpp#L1709)) asks one HTTP suffix-range question: slicers write `filament used [g]` and the color palette *after the last move*, so the last few tens of KB answer "which tools does this file use?" with no whole-file download and no geometry parse. The window is `gcode_tail_window_bytes()` ([`include/gcode_footer_summary.h:127`](../../../include/gcode_footer_summary.h#L127)) — exactly `size - gcode_end_byte` when Moonraker's metadata reported the footer offset, clamped to `[16KB, 512KB]`, defaulting to 64KB when it didn't.
...
8. [`src/ui/ui_print_select_detail_view.cpp:1709`](../../../src/ui/ui_print_select_detail_view.cpp#L1709) — `start_tail_summary_scan()`: the suffix request, pure parse on the HTTP thread, stale-file guard, and fall-through.
...
11. [`src/ui/ui_panel_print_status.cpp:1061`](../../../src/ui/ui_panel_print_status.cpp#L1061) — the precedence chain applied; then the pinned-mode guard at `:346`.
