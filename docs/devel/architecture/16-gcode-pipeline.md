...
The precedence chain is **cmdline > env > settings**, applied in the panel's `on_activate` ([`src/ui/ui_panel_print_status.cpp:1116`](../../../src/ui/ui_panel_print_status.cpp#L1116)): `--render-2d`/`--render-3d` (stored in `RuntimeConfig::gcode_render_mode`) win; otherwise `HELIX_GCODE_MODE` — resolved at widget creation by the pure `decide_render_mode()` ([`include/gcode_render_mode_policy.h:44`](../../../include/gcode_render_mode_policy.h#L44)) — stands; otherwise the saved setting is applied. Two traps are load-bearing:

- The settings observer would otherwise fire once at startup with the persisted value and silently overwrite the cmdline pin ~16ms after it was applied; it now stands down when the cmdline pinned a mode ([`src/ui/ui_panel_print_status.cpp:391`](../../../src/ui/ui_panel_print_status.cpp#L391)).
...
11. [`src/ui/ui_panel_print_status.cpp:1116`](../../../src/ui/ui_panel_print_status.cpp#L1116) — the precedence chain applied; then the pinned-mode guard at `:346`.
