...
`ui_gcode_viewer_load_file_async()` decides once per file, in "Phase 0", before any UI appears ([`src/ui/ui_gcode_viewer.cpp:1505`](../../../src/ui/ui_gcode_viewer.cpp#L1505)): streaming when `should_use_gcode_streaming(file_size)` says so and the host hasn't disabled it. That predicate ([`include/gcode_streaming_config.h:71`](../../../include/gcode_streaming_config.h#L71)) resolves a three-step hierarchy — `HELIX_GCODE_STREAMING` env var, then `gcode_viewer.streaming_mode` from config, then AUTO, which compares file size against a threshold computed from available memory and a configured percentage (default 40%).
...
2. [`src/ui/ui_gcode_viewer.cpp:1505`](../../../src/ui/ui_gcode_viewer.cpp#L1505) — Phase 0: the streaming decision in five lines, then the two paths diverging at [`src/ui/ui_gcode_viewer.cpp:1574`](../../../src/ui/ui_gcode_viewer.cpp#L1574) (streaming) and [`src/ui/ui_gcode_viewer.cpp:1724`](../../../src/ui/ui_gcode_viewer.cpp#L1724) (full-load).
...
13. [`src/ui/ui_gcode_viewer.cpp:866`](../../../src/ui/ui_gcode_viewer.cpp#L866) — the long-press constant and its comment; then the release callback's tap handling at `:1083`.
