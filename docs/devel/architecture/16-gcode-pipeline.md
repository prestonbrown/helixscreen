...
The **detail view** has one canonical entry point, `ensure_gcode_downloaded()` ([`src/ui/ui_print_select_detail_view.cpp:611`](../../../src/ui/ui_print_select_detail_view.cpp#L611)), because two consumers need the file — the viewer preview and the tools scan — and they must not race each other into two transfers. Its order matters:
...
5. [`src/ui/ui_print_select_detail_view.cpp:611`](../../../src/ui/ui_print_select_detail_view.cpp#L611) — `ensure_gcode_downloaded()`: the local-read/join/disk/start order and why each precedes the next.
6. [`src/ui/ui_print_select_detail_view.cpp:566`](../../../src/ui/ui_print_select_detail_view.cpp#L566) — `local_gcode_source()`: the same-host probe and size staleness check; then `reclaim_download()` at `:627` for the delete gate.
...
8. [`src/ui/ui_print_select_detail_view.cpp:1739`](../../../src/ui/ui_print_select_detail_view.cpp#L1739) — `start_tail_summary_scan()`: the local-vs-HTTP tail read, pure parse off the main thread, stale-file guard, and the need-dependent fall-through.
9. [`include/tools_used_cache.h:20`](../../../include/tools_used_cache.h#L20) — the cache: keying, LRU bound, and the "empty set is legitimate" contract at `:24`. Then its three call sites in the detail view ([`src/ui/ui_print_select_detail_view.cpp:455`](../../../src/ui/ui_print_select_detail_view.cpp#L455), [`src/ui/ui_print_select_detail_view.cpp:1253`](../../../src/ui/ui_print_select_detail_view.cpp#L1253), [`src/ui/ui_print_select_detail_view.cpp:1638`](../../../src/ui/ui_print_select_detail_view.cpp#L1638)).
