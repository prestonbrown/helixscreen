...
| [`scripts/quality-checks.sh`](../../../scripts/quality-checks.sh) | CI entry that runs the gate with `--max-allowed 367` (`:1601`) — the number to ratchet down |
...
The declarative target is the shape every [`ui_xml/bed_temp_panel.xml#preset_m0`](../../../ui_xml/bed_temp_panel.xml#L73) preset button already uses — declare the callback where the button is declared ([`ui_xml/gcode_test_panel.xml#btn_zoom_in`](../../../ui_xml/gcode_test_panel.xml#L61) defines `btn_zoom_in` today, callback-less):
...
and publish the handler by name from C++ — either a `{"name", fn}` table like [`src/ui/temperature_service.cpp:205`](../../../src/ui/temperature_service.cpp#L205) or a direct `lv_xml_register_event_cb()` as [`src/xml_registration.cpp#register_xml_components`](../../../src/xml_registration.cpp#L331) does. The static wrapper, the null-check, and the ledger entry all disappear.
...
| **Per-item payload on generated collections** | `lv_obj_set_user_data()` on a `ui_button` overwrites the `UiButtonData*` it owns ([`src/ui/temperature_service.cpp#setup_panel`](../../../src/ui/temperature_service.cpp#L669) warns at the site) |
...
- **The port workflow is two edits.** Port the site, then lower the number in [`scripts/quality-checks.sh#qc_decl_ui`](../../../scripts/quality-checks.sh#L1601) (and root [`AGENTS.md`](../../../AGENTS.md), if you keep it in sync) in the same commit. The gate output tells you the new total.
...
1. [`scripts/check_imperative_ui.py`](../../../scripts/check_imperative_ui.py#L10) — the header comment: what is flagged, what is structurally exempt, and the ratchet philosophy. The whole chapter in 40 lines.
2. [`scripts/quality-checks.sh#qc_decl_ui`](../../../scripts/quality-checks.sh#L1601) — where the baseline 367 is enforced and how a port ratchets it down.
...
6. [`src/ui/panel_widgets/fan_stack_widget.cpp#bind_fan_observer`](../../../src/ui/panel_widgets/fan_stack_widget.cpp#L688) — `bind_fan_observer()`: the manual subject read that works around the deferred initial fire under populate's freeze.
...
12. [`src/ui/temperature_service.cpp#setup_panel`](../../../src/ui/temperature_service.cpp#L669) — the in-code warning that documents the `user_data` toleration row better than any doc could.
