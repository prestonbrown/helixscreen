...
flowchart TD
    ...
    GATE["scripts/check_imperative_ui.py<br/>baseline 371, enforced in<br/>scripts/quality-checks.sh:1583"]
...
| [`scripts/quality-checks.sh`](../../../scripts/quality-checks.sh) | CI entry that runs the gate with `--max-allowed 371` (`:1583`) — the number to ratchet down |
...
2. [`scripts/quality-checks.sh:1583`](../../../scripts/quality-checks.sh#L1583) — where the baseline 371 is enforced and how a port ratchets it down.
