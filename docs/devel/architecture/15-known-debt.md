...
flowchart TD
    ...
    subgraph LEDGER["The ledger — 370 sites (issue #1140)"]
        V["visibility 148"]
        T["text 93"]
        S["style 63"]
        E["event 67"]
    ...
    GATE["scripts/check_imperative_ui.py<br/>baseline 370, enforced in<br/>scripts/quality-checks.sh:1603"]
...
| [`scripts/quality-checks.sh`](../../../scripts/quality-checks.sh) | CI entry that runs the gate with `--max-allowed 370` (`:1603`) — the number to ratchet down |
...
(verbatim from `python3 scripts/check_imperative_ui.py --summary`; regenerate any number in this section with `--list`). The count is enforced as a ratchet: [`scripts/quality-checks.sh:1601`](../../../scripts/quality-checks.sh#L1601) runs the gate with `--max-allowed 370`, so a change that adds even one site fails CI, and a port lowers both the count and the baseline. The debt is tracked in prestonbrown/helixscreen#1140. (An earlier revision of root [`AGENTS.md`](../../../AGENTS.md) said 387 — that number counted the report's own header and summary lines. The script's `TOTAL` is authoritative; root now cites it.)

Where the 370 lives, by directory:

| Area | Sites | Notes |
|------|-------|-------|
| `src/ui/` flat files | 326 | panels, overlays, wizards, services |
...
The gate does not merely tolerate these cases — it excludes them structurally, so they never appear in the 370: files that call `lv_xml_register_widget` are skipped whole, widgets created with `lv_*_create` in C++ never had an XML layer, events with no declarative equivalent (`DELETE`, draw hooks, size/scroll) are not flagged, and neither are annotated lines. The table (verified against the root [`AGENTS.md`](../../../AGENTS.md) and the code — the bolded entries were spot-checked for this chapter):
...
- **Existing imperative code is not precedent.** The 370 sites are bounded debt, not an alternative style. A nearby `lv_label_set_text()` never justifies yours.
...
- **The port workflow is two edits.** Port the site, then lower the number in [`scripts/quality-checks.sh:1601`](../../../scripts/quality-checks.sh#L1601) (and root [`AGENTS.md`](../../../AGENTS.md), if you keep it in sync) in the same commit. The gate output tells you the new total.
...
2. [`scripts/quality-checks.sh:1601`](../../../scripts/quality-checks.sh#L1601) — where the baseline 370 is enforced and how a port ratchets it down.
