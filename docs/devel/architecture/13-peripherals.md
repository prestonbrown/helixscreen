...
elsewhere; [`application.cpp:1021`](../../../src/application/application.cpp#L1021)), is compiled in only when `ENABLE_REMOTE_CONTROL=yes` — default ON for every
...
- **BT callbacks arrive on background threads — the settings screens show the pattern.** Discovery results and pairing state are marshaled with `helix::ui::queue_update()` plus a lifetime token at every consumer site ([`src/ui/ui_settings_label_printer.cpp:1118`](../../../src/ui/ui_settings_label_printer.cpp#L1118), `:1356`); a new BT-adjacent UI that skips the token is an L081-shaped bug.
