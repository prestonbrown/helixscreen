...
| `PrintHistoryManager` | Application-owned ([`src/application/application.cpp#init_moonraker`](../../../src/application/application.cpp#L2109)), published as nullable pointer ([`include/app_globals.h#get_print_history_manager`](../../../include/app_globals.h#L116)) | no | no (client callbacks) | none |
...
| `PluginManager` | Application-owned `unique_ptr` ([`src/application/application.cpp#init_plugins`](../../../src/application/application.cpp#L2150)) — **not** `::instance()` | no | plugins may spawn their own | [`../PLUGIN_DEVELOPMENT.md`](../PLUGIN_DEVELOPMENT.md) |
...
|------------|----------------|--------------------------------------------------|
| 9b | UpdateChecker, UpgradeBanner, CrashHistory + CrashReporter, TelemetryManager, PrintHistoryManager | `:776`, `:782`, `:791`–`:797`, `:2102` |
| 9d follow-on | SoundManager (backend pick + startup tone), PostOpCooldownManager | `:841`, `:856` |
| 11b | CrashReportModal — only if a pending crash survives suppression | `:889`–`:916` |
| during connect | Timelapse subscription; telemetry `start_auto_send()` | `:2962`, `:3498` |
| 14 | PluginManager discover + load | `:2143` |
| 15 | MemoryMonitor sampling, hang detector, pressure responders | `:1029`–`:1104` |

### Update & recovery

**UpdateChecker** is a singleton with two worker threads: `worker_thread_` ([`src/system/update_checker.cpp#check_for_updates`](../../../src/system/update_checker.cpp#L2580)), joined in the destructor, and `download_thread_` (`:1341`), which is *not* — it is bounded-then-detached by `reap_download_thread()` (`:1374`) because libhv's synchronous `downloadFile()` cannot be interrupted. Chapter 03 has the full reasoning. It polls a release manifest on the R2 CDN at `releases.helixscreen.org` (GitHub releases API as fallback — chapter 04 has the endpoint story) on three channels: Stable, Beta, Dev (`UpdateChannel`, [`include/system/update_checker.h#UpdateChannel`](../../../include/system/update_checker.h#L116)). Checks are rate-limited to `MIN_CHECK_INTERVAL = 10 minutes` ([`include/system/update_checker.h#MIN_CHECK_INTERVAL`](../../../include/system/update_checker.h#L576) — the old monolith's "1 check per hour" was stale); auto-check runs 15s after boot, then every 24h ([`include/system/update_checker.h#UpdateChecker`](../../../include/system/update_checker.h#L450)). The install ladder is the `DownloadStatus` enum ([`include/system/update_checker.h#DownloadStatus`](../../../include/system/update_checker.h#L121)):

| Step | What happens | Guard |
|------|--------------|-------|
| Confirming | User confirms the release-notes modal | Downgrades offered explicitly, never auto-notified (`is_downgrade`, [`include/system/update_checker.h#ReleaseInfo`](../../../include/system/update_checker.h#L96)) |
| Downloading | `download_thread_` fetches the platform zip | Blocked while `PRINTING`/`PAUSED` (`:1268`, reported to telemetry); re-entry blocked while a previous worker is alive (`:1315`) |
| Verifying | archive integrity (`:1537`) + ELF architecture check (`:1567`) + SHA-256 when the manifest supplies one | Wrong-arch or corrupt archive aborts before install |
| Installing | [`install.sh`](../../../scripts/install.sh) via `safe_exec()` fork/exec — no shell (`:407`) | Space pre-check; systemd `NoNewPrivileges` workaround (`:505`) |
| Complete → Restarting | `finish_install_and_restart()` (`:2251`) holds the Complete frame 2s, publishes `Restarting`, holds 1s, then hands `::_exit(0)` to the LVGL thread via `queue_update()` | The holds exist because `report_download_status()` only *queues* its subject write — exiting straight after it left "Installing..." as the last painted frame |
...
Cancel is **deferred**, and `DownloadStatus` does not model it. `cancel_download()` (`:1360`) sets a flag the worker reads only after `downloadFile()` returns, while the modal resets the status to `Idle` the moment the user taps Cancel — so `Idle` can mean "a worker is still inside an hour-long blocking call". `download_in_flight()` is the authoritative answer, and `start_download()` gates re-entry on it (`:1315`), refusing with an `Error` rather than joining. Every early return from `start_download()` must report *some* non-`Idle` status: [`ui_xml/update_download_modal.xml`](../../../ui_xml/update_download_modal.xml) binds every container and every button row to a non-zero `download_status`, so a silent return renders an empty dialog with no buttons.

**CrashReporter** owns no thread (chapter 03 corrected the old diagram on this). The crash handler writes `<config/crash.txt>` at death time; on the next healthy launch, phase 11b decides whether to show `CrashReportModal` ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L899)–`:916`) — three guards suppress the dialog first: a crash from a post-update restart, an unparseable record (signal handler killed mid-write — OOM-killer, power loss), and duplicate fingerprints. `try_auto_send()` ([`src/system/crash_reporter.cpp#try_auto_send`](../../../src/system/crash_reporter.cpp#L998)) POSTs the report JSON to a Cloudflare Worker at `crash.helixscreen.org/v1/report`, which files a GitHub issue; a failed send leaves the file so the modal can offer manual retry. Handled records are deleted to prevent re-processing ([`include/system/crash_reporter.h#CrashReporter`](../../../include/system/crash_reporter.h#L233)). Delivery rides existing HTTP paths — on Android, a JNI bridge to Java's `HttpURLConnection` ([`src/system/crash_reporter.cpp#android_https_post`](../../../src/system/crash_reporter.cpp#L917)) — which is *why* it needs no thread of its own. The watchdog half of crash recovery — exit-code translation, the recovery dialog, Safe Mode — is chapter 11.

Adjacent to the updater sits **UpgradeBanner** (`::instance()`, [`include/upgrade_banner.h`](../../../include/upgrade_banner.h)), a persistent top-banner widget initialized at phase 9b ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L792)) that nudges users toward a pending release with an intensity setting (`/upgrade_nudge/intensity`) that defaults to `off`. It is update *messaging*, not update *machinery* — the download pipeline never goes through it.

### Feedback channels

**SoundManager** auto-detects a host-side backend in `create_backend()` ([`src/system/sound_manager.cpp#create_backend`](../../../src/system/sound_manager.cpp#L377)), then installs M300 separately once discovery speaks:
...
| M300 | **Lazily**, after hardware discovery confirms a speaker *and* the Klipper config defines an `M300` macro ([`src/system/sound_manager.cpp#initialize`](../../../src/system/sound_manager.cpp#L90)–`128`) | Klipper gcode through the injected Moonraker client |

Installing M300 eagerly on a beeper-less printer creates a feedback loop (`M300` → `!! Unknown command` → error toast → error tone → another `M300`). The Moonraker client used by M300 is injected at [`src/application/moonraker_manager.cpp#create_client`](../../../src/application/moonraker_manager.cpp#L435) and dropped when the client clears ([`src/system/sound_manager.cpp#set_moonraker_client`](../../../src/system/sound_manager.cpp#L54) — switching printers must not carry M300 over to a machine without a beeper). ALSA device choice is lockable via `HELIX_ALSA_DEVICE`; alarm-priority sounds bypass muting ([`sound_manager.cpp:265`](../../../src/system/sound_manager.cpp#L265)). Sound *themes* are JSON in `assets/config/sounds/`; on printer switch the manager is shut down and re-initialized ([`src/application/application.cpp#on_enter_background`](../../../src/application/application.cpp#L4390), `:4384`) so the backend set matches the new machine.

**LED** control is `LedController` (singleton, self-registers its `deinit_subjects`, [`src/led/led_controller.cpp#init`](../../../src/led/led_controller.cpp#L78)) fronting five backends: Native (Klipper `neopixel`/`dotstar`/`led`), LedEffect (the `led_effect` plugin), WLED (network strips via Moonraker's HTTP bridge), Macro (user-configured macros), and OutputPin (brightness-only pins). `LedAutoState` ([`include/led/led_auto_state.h`](../../../include/led/led_auto_state.h)) observes klippy-state, print-state, and extruder-target subjects and computes one of six state keys with a fixed priority — `error` first, then `printing`/`paused`/`complete`, then `heating`, finally `idle` (`compute_state_key`, [`src/led/led_auto_state.cpp#compute_state_key`](../../../src/led/led_auto_state.cpp#L112)) — and applies the user's per-state mapping (`color`, `brightness`, `effect`, `wled_preset`, `macro`, or `off`). `PrinterLedState` ([`include/printer_led_state.h`](../../../include/printer_led_state.h)) is the display-side sibling: per-strip RGBW subjects that feed the LED home-panel widget.

**Notifications and toasts** are two singletons with different jobs. `ToastManager` is the transient channel: toasts stack in the top-right, each with its own dismiss timer and optional action button, and the stack caps at `MAX_VISIBLE = 5` — overflow **silently drops the oldest**, no queueing ([`include/ui_toast_manager.h`](../../../include/ui_toast_manager.h#L30)–`35`). Roughly 56 files under `src/` call it, which is the point: any service can raise a toast without owning UI. `NotificationManager` is the persistent channel: navbar badge (unread count, severity color, pulse) plus the history panel. It registers three subjects — `notification_count`, `notification_count_text`, `notification_severity` ([`include/ui_notification_manager.h#NotificationManager`](../../../include/ui_notification_manager.h#L60)–`63`) — and its ordering contract is strict: `register_callbacks()` and `init_subjects()` before `app_layout` XML is created, `init()` after (`:29`–`32`).

### Ambient monitoring

**TelemetryManager** is opt-in and **OFF by default** — it only activates after the user enables it in settings ([`include/system/telemetry_manager.h`](../../../include/system/telemetry_manager.h#L102)). Events (session, print outcomes, memory warnings, update failures, errors) are recorded thread-safely, persisted to disk so they survive restarts, capped at `MAX_QUEUE_SIZE` (oldest dropped), and sent as batches on a per-batch thread to `telemetry.helixscreen.org/v1/events` (chapter 04). Startup spreads three calls across the boot ladder (chapter 11): `init()` at phase 9b, `record_session()` on discovery completion, `start_auto_send()` inside `connect_moonraker()`.

**MemoryMonitor** samples `/proc/self/status` every 5 seconds ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L1037), phase 15) on its own thread and classifies pressure into four levels — `none`/`elevated`/`warning`/`critical` ([`include/memory_monitor.h#MemoryPressureLevel`](../../../include/memory_monitor.h#L37)) — against device-tier thresholds with hysteresis (levels clear at 90% of the trigger, [`src/system/memory_monitor.cpp#for_device`](../../../src/system/memory_monitor.cpp#L67)). Its consumers, all wired in phase 15:
...
Both `critical` responders fire on the monitor thread and defer the actual LVGL calls through `queue_update()` ([`src/application/application.cpp#run`](../../../src/application/application.cpp#L1084)–`1091`). Outside pressure monitoring, MemoryMonitor also takes heap snapshots at boot milestones (`log_now("after_fonts_loaded")`, etc. — [`src/application/application.cpp#run`](../../../src/application/application.cpp#L1162)–`1856`) that anchor the memory stats panel.
...
- **Subject-owning services self-register their `deinit_subjects()`** — LedController does it at the end of `init()` ([`src/led/led_controller.cpp#init`](../../../src/led/led_controller.cpp#L78)), NotificationManager exposes its own for the same reason. A new service with XML-bound subjects joins `StaticSubjectRegistry` itself; nobody registers it from outside (chapters 02 and 05 own the rule).
...
- **Moonraker-method subscriptions are a shutdown obligation** — `notify_timelapse_event` and `notify_history_changed` handlers are unregistered in `Application::shutdown()` ([`src/application/application.cpp#tear_down_printer_state`](../../../src/application/application.cpp#L4842), `:5131`); a new service subscribing to a notification must add its unregister or crash teardown.
- **Services stop early in the shutdown ladder.** Step 1 (chapter 11) clears the service managers — UpdateChecker, Telemetry, CrashHistory, Sound, plugins — *before* panels and subjects die, so a service's teardown must not depend on either. `SoundManager::shutdown()` ([`src/application/application.cpp#shutdown`](../../../src/application/application.cpp#L5112)) joins the sequencer thread here.
...
2. [`src/system/update_checker.cpp#start_download`](../../../src/system/update_checker.cpp#L1241) — the print-guard block and its telemetry report; then `:1317` where `download_thread_` spawns and `:1341` for the re-entry guard.
3. [`src/system/sound_manager.cpp#create_backend`](../../../src/system/sound_manager.cpp#L377) — `create_backend()`: the SDL → ALSA → PWM ladder and the per-platform PWM gating comment.
4. [`src/system/sound_manager.cpp#initialize`](../../../src/system/sound_manager.cpp#L97) — the M300 install gate and the feedback-loop comment above it.
...
6. [`include/memory_monitor.h#MemoryPressureLevel`](../../../include/memory_monitor.h#L37) — `MemoryPressureLevel` and `MemoryThresholds`: levels, tiers, hysteresis fields.
7. [`src/application/application.cpp#run`](../../../src/application/application.cpp#L1037) — phase 15 wiring: `start(5000)`, the warning callback, hang detection, both pressure responders.
8. [`include/system/telemetry_manager.h`](../../../include/system/telemetry_manager.h#L20) — the block-comment architecture diagram (collect → persist → batch → POST) and the thread contract.
9. [`include/post_op_cooldown_manager.h`](../../../include/post_op_cooldown_manager.h#L8) — the entire header; the threading and opt-out rules are in the doc comments.
10. [`src/application/application.cpp#run`](../../../src/application/application.cpp#L899) — the crash-dialog gate: the three suppression paths (post-update, unparseable, duplicate) before `CrashReportModal`.
11. [`src/application/application.cpp#init_plugins`](../../../src/application/application.cpp#L2147) — `init_plugins()`: owned manager, service injection, the error toast with Disable action.
...
14. [`include/ui_toast_manager.h`](../../../include/ui_toast_manager.h#L30) — the stack semantics comment, then [`include/ui_notification_manager.h`](../../../include/ui_notification_manager.h#L29) for the before/after-XML ordering contract.
