...
| `PrintHistoryManager` | Application-owned ([`application.cpp:2102`](../../../src/application/application.cpp#L2102)), published as nullable pointer ([`app_globals.h:116`](../../../include/app_globals.h#L116)) | no | no (client callbacks) | none |
...
| `PluginManager` | Application-owned `unique_ptr` ([`application.cpp:2143`](../../../src/application/application.cpp#L2143)) — **not** `::instance()` | no | plugins may spawn their own | [`../PLUGIN_DEVELOPMENT.md`](../PLUGIN_DEVELOPMENT.md) |
...
Both `critical` responders fire on the monitor thread and defer the actual LVGL calls through `queue_update()` ([`application.cpp:1076`](../../../src/application/application.cpp#L1076)–`1091`). Outside pressure monitoring, MemoryMonitor also takes heap snapshots at boot milestones (`log_now("after_fonts_loaded")`, etc. — [`application.cpp:1154`](../../../src/application/application.cpp#L1154)–`1856`) that anchor the memory stats panel.
...
- **Moonraker-method subscriptions are a shutdown obligation** — `notify_timelapse_event` and `notify_history_changed` handlers are unregistered in `Application::shutdown()` ([`application.cpp:4832`](../../../src/application/application.cpp#L4832), `:5131`); a new service subscribing to a notification must add its unregister or crash teardown.
- **Services stop early in the shutdown ladder.** Step 1 (chapter 11) clears the service managers — UpdateChecker, Telemetry, CrashHistory, Sound, plugins — *before* panels and subjects die, so a service's teardown must not depend on either. `SoundManager::shutdown()` ([`application.cpp:5101`](../../../src/application/application.cpp#L5101)) joins the sequencer thread here.
...
7. [`src/application/application.cpp:1029`](../../../src/application/application.cpp#L1029) — phase 15 wiring: `start(5000)`, the warning callback, hang detection, both pressure responders.
...
10. [`src/application/application.cpp:889`](../../../src/application/application.cpp#L889) — the crash-dialog gate: the three suppression paths (post-update, unparseable, duplicate) before `CrashReportModal`.
11. [`src/application/application.cpp:2140`](../../../src/application/application.cpp#L2140) — `init_plugins()`: owned manager, service injection, the error toast with Disable action.
