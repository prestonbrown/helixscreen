...
| M300 | **Lazily**, after hardware discovery confirms a speaker *and* the Klipper config defines an `M300` macro ([`sound_manager.cpp:124`](../../../src/system/sound_manager.cpp#L124)–`128`); a real beeper signal also displaces the PWM sysfs backend — klippy's tone_player writes the same buzzer channel | Klipper gcode through the injected Moonraker client |
