...
- **Moonraker-method subscriptions are a teardown obligation, and the unit is the handler name, not the method** - `src/application/application.cpp#tear_down_printer_state` and `src/application/application.cpp#shutdown` both unregister `timelapse_state` and `AboutOverlay_print_hours`, the handlers `on_discovery_complete` installs; `external_update_restart` is dropped on the printer-switch path only. One method can carry several independently-owned handlers, so a method being unregistered somewhere is no evidence that a given handler is - `notify_history_changed` also carries `PrintHistoryManager`, released by that manager's own destructor. A new service subscribing to a notification must add its own unregister to both paths: the client is released well after the panels its callback reaches, so a handler left registered outlives the state it touches.
