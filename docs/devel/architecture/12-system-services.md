...
| `PrintHistoryManager` | Application-owned ([`application.cpp:2112`](../../../src/application/application.cpp#L2112)), published as nullable pointer ([`app_globals.h:116`](../../../include/app_globals.h#L116)) | no | no (client callbacks) | none |
...
| `PluginManager` | Application-owned `unique_ptr` ([`application.cpp:2153`](../../../src/application/application.cpp#L2153)) — **not** `::instance()` | no | plugins may spawn their own | [`../PLUGIN_DEVELOPMENT.md`](../PLUGIN_DEVELOPMENT.md) |
...
Installing M300 eagerly on a beeper-less printer creates a feedback loop (`M300` → `!! Unknown command` → error toast → error tone → another `M300`). The Moonraker client used by M300 is injected at [`moonraker_manager.cpp:436`](../../../src/application/moonraker_manager.cpp#L436) and dropped when the client clears ([`sound_manager.cpp:54`](../../../src/system/sound_manager.cpp#L54) — switching printers must not carry M300 over to a machine without a beeper). ALSA device choice is lockable via `HELIX_ALSA_DEVICE`; alarm-priority sounds bypass muting ([`sound_manager.cpp:265`](../../../src/system/sound_manager.cpp#L265)). Sound *themes* are JSON in `assets/config/sounds/`; on printer switch the manager is shut down and re-initialized ([`application.cpp:4397`](../../../src/application/application.cpp#L4397), `:4384`) so the backend set matches the new machine.
...
Both `critical` responders fire on the monitor thread and defer the actual LVGL calls through `queue_update()` ([`application.cpp:1086`](../../../src/application/application.cpp#L1086)–`1091`). Outside pressure monitoring, MemoryMonitor also takes heap snapshots at boot milestones (`log_now("after_fonts_loaded")`, etc. — [`application.cpp:1164`](../../../src/application/application.cpp#L1164)–`1856`) that anchor the memory stats panel.
...
- **Moonraker-method subscriptions are a shutdown obligation** — `notify_timelapse_event` and `notify_history_changed` handlers are unregistered in `Application::shutdown()` ([`application.cpp:4849`](../../../src/application/application.cpp#L4849), `:5131`); a new service subscribing to a notification must add its unregister or crash teardown.
- **Services stop early in the shutdown ladder.** Step 1 (chapter 11) clears the service managers — UpdateChecker, Telemetry, CrashHistory, Sound, plugins — *before* panels and subjects die, so a service's teardown must not depend on either. `SoundManager::shutdown()` ([`application.cpp:5119`](../../../src/application/application.cpp#L5119)) joins the sequencer thread here.
...
2. [`src/system/update_checker.cpp:1245`](../../../src/system/update_checker.cpp#L1245) — the print-guard block and its telemetry report; then `:1315` for the re-entry guard and `:1341` where `download_thread_` spawns.
...
7. [`src/application/application.cpp:1039`](../../../src/application/application.cpp#L1039) — phase 15 wiring: `start(5000)`, the warning callback, hang detection, both pressure responders.
...
10. [`src/application/application.cpp:899`](../../../src/application/application.cpp#L899) — the crash-dialog gate: the three suppression paths (post-update, unparseable, duplicate) before `CrashReportModal`.
11. [`src/application/application.cpp:2150`](../../../src/application/application.cpp#L2150) — `init_plugins()`: owned manager, service injection, the error toast with Disable action.
