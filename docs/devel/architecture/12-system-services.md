...
| `PrintHistoryManager` | Application-owned ([`application.cpp:2154`](../../../src/application/application.cpp#L2154)), published as nullable pointer ([`app_globals.h:116`](../../../include/app_globals.h#L116)) | no | no (client callbacks) | none |
...
| `PluginManager` | Application-owned `unique_ptr` ([`application.cpp:2195`](../../../src/application/application.cpp#L2195)) — **not** `::instance()` | no | plugins may spawn their own | [`../PLUGIN_DEVELOPMENT.md`](../PLUGIN_DEVELOPMENT.md) |
...
| M300 | **Lazily**, after hardware discovery confirms a speaker *and* the Klipper config defines an `M300` macro ([`sound_manager.cpp:125`](../../../src/system/sound_manager.cpp#L125)–`128`) | Klipper gcode through the injected Moonraker client |

Installing M300 eagerly on a beeper-less printer creates a feedback loop (`M300` → `!! Unknown command` → error toast → error tone → another `M300`). The Moonraker client used by M300 is injected at [`moonraker_manager.cpp:451`](../../../src/application/moonraker_manager.cpp#L451) and dropped when the client clears ([`sound_manager.cpp:88`](../../../src/system/sound_manager.cpp#L88) — switching printers must not carry M300 over to a machine without a beeper). ALSA device choice is lockable via `HELIX_ALSA_DEVICE`; alarm-priority sounds bypass muting ([`sound_manager.cpp:290`](../../../src/system/sound_manager.cpp#L290)). Sound *themes* are JSON in `assets/config/sounds/`; on printer switch the manager is shut down and re-initialized ([`application.cpp:4439`](../../../src/application/application.cpp#L4439), `:4384`) so the backend set matches the new machine.
...
**MemoryMonitor** samples `/proc/self/status` every 5 seconds ([`application.cpp:1038`](../../../src/application/application.cpp#L1038), phase 15) on its own thread and classifies pressure into four levels — `none`/`elevated`/`warning`/`critical` ([`include/memory_monitor.h:37`](../../../include/memory_monitor.h#L37)) — against device-tier thresholds with hysteresis (levels clear at 90% of the trigger, [`src/system/memory_monitor.cpp:67`](../../../src/system/memory_monitor.cpp#L67)). Its consumers, all wired in phase 15:
...
Both `critical` responders fire on the monitor thread and defer the actual LVGL calls through `queue_update()` ([`application.cpp:1085`](../../../src/application/application.cpp#L1085)–`1091`). Outside pressure monitoring, MemoryMonitor also takes heap snapshots at boot milestones (`log_now("after_fonts_loaded")`, etc. — [`application.cpp:1163`](../../../src/application/application.cpp#L1163)–`1856`) that anchor the memory stats panel.
...
- **Moonraker-method subscriptions are a shutdown obligation** — `notify_timelapse_event` and `notify_history_changed` handlers are unregistered in `Application::shutdown()` ([`application.cpp:4891`](../../../src/application/application.cpp#L4891), `:5131`); a new service subscribing to a notification must add its unregister or crash teardown.
- **Services stop early in the shutdown ladder.** Step 1 (chapter 11) clears the service managers — UpdateChecker, Telemetry, CrashHistory, Sound, plugins — *before* panels and subjects die, so a service's teardown must not depend on either. `SoundManager::shutdown()` ([`application.cpp:5163`](../../../src/application/application.cpp#L5163)) joins the sequencer thread here.
...
2. [`src/system/update_checker.cpp:1242`](../../../src/system/update_checker.cpp#L1242) — the print-guard block and its telemetry report; then `:1317` where `download_thread_` spawns and `:1341` for the re-entry guard.
3. [`src/system/sound_manager.cpp:402`](../../../src/system/sound_manager.cpp#L402) — `create_backend()`: the SDL → ALSA → PWM ladder and the per-platform PWM gating comment.
4. [`src/system/sound_manager.cpp:171`](../../../src/system/sound_manager.cpp#L171) — the M300 install gate and the feedback-loop comment above it.
...
10. [`src/application/application.cpp:900`](../../../src/application/application.cpp#L900) — the crash-dialog gate: the three suppression paths (post-update, unparseable, duplicate) before `CrashReportModal`.
11. [`src/application/application.cpp:2192`](../../../src/application/application.cpp#L2192) — `init_plugins()`: owned manager, service injection, the error toast with Disable action.
12. [`src/print/print_history_manager.cpp:290`](../../../src/print/print_history_manager.cpp#L290) — the `notify_history_changed` registration and `token.defer()` invalidation.
