...
| `PrintHistoryManager` | Application-owned ([`application.cpp:2148`](../../../src/application/application.cpp#L2148)), published as nullable pointer ([`app_globals.h:116`](../../../include/app_globals.h#L116)) | no | no (client callbacks) | none |
...
| `PluginManager` | Application-owned `unique_ptr` ([`application.cpp:2189`](../../../src/application/application.cpp#L2189)) — **not** `::instance()` | no | plugins may spawn their own | [`../PLUGIN_DEVELOPMENT.md`](../PLUGIN_DEVELOPMENT.md) |
...
Both `critical` responders fire on the monitor thread and defer the actual LVGL calls through `queue_update()` ([`application.cpp:1078`](../../../src/application/application.cpp#L1078)–`1091`). Outside pressure monitoring, MemoryMonitor also takes heap snapshots at boot milestones (`log_now("after_fonts_loaded")`, etc. — [`application.cpp:1156`](../../../src/application/application.cpp#L1156)–`1856`) that anchor the memory stats panel.
...
7. [`src/application/application.cpp:1031`](../../../src/application/application.cpp#L1031) — phase 15 wiring: `start(5000)`, the warning callback, hang detection, both pressure responders.
...
10. [`src/application/application.cpp:891`](../../../src/application/application.cpp#L891) — the crash-dialog gate: the three suppression paths (post-update, unparseable, duplicate) before `CrashReportModal`.
11. [`src/application/application.cpp:2186`](../../../src/application/application.cpp#L2186) — `init_plugins()`: owned manager, service injection, the error toast with Disable action.
