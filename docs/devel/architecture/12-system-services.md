...
| `PrintHistoryManager` | Application-owned ([`application.cpp:2109`](../../../src/application/application.cpp#L2109)), published as nullable pointer ([`app_globals.h:116`](../../../include/app_globals.h#L116)) | no | no (client callbacks) | none |
...
| `PluginManager` | Application-owned `unique_ptr` ([`application.cpp:2150`](../../../src/application/application.cpp#L2150)) — **not** `::instance()` | no | plugins may spawn their own | [`../PLUGIN_DEVELOPMENT.md`](../PLUGIN_DEVELOPMENT.md) |
...
Both `critical` responders fire on the monitor thread and defer the actual LVGL calls through `queue_update()` ([`application.cpp:1084`](../../../src/application/application.cpp#L1084)–`1091`). Outside pressure monitoring, MemoryMonitor also takes heap snapshots at boot milestones (`log_now("after_fonts_loaded")`, etc. — [`application.cpp:1162`](../../../src/application/application.cpp#L1162)–`1856`) that anchor the memory stats panel.
...
- **Moonraker-method subscriptions are a shutdown obligation** — `notify_timelapse_event` and `notify_history_changed` handlers are unregistered in `Application::shutdown()` ([`application.cpp:4843`](../../../src/application/application.cpp#L4843), `:5131`); a new service subscribing to a notification must add its unregister or crash teardown.
- **Services stop early in the shutdown ladder.** Step 1 (chapter 11) clears the service managers — UpdateChecker, Telemetry, CrashHistory, Sound, plugins — *before* panels and subjects die, so a service's teardown must not depend on either. `SoundManager::shutdown()` ([`application.cpp:5113`](../../../src/application/application.cpp#L5113)) joins the sequencer thread here.
...
7. [`src/application/application.cpp:1037`](../../../src/application/application.cpp#L1037) — phase 15 wiring: `start(5000)`, the warning callback, hang detection, both pressure responders.
...
11. [`src/application/application.cpp:2147`](../../../src/application/application.cpp#L2147) — `init_plugins()`: owned manager, service injection, the error toast with Disable action.
