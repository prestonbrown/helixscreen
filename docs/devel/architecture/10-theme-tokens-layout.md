...
The user-facing controls live in Settings → Display: `DisplaySettingsManager` ([`src/system/display_settings_manager.cpp:434`](../../../src/system/display_settings_manager.cpp#L434)) guards against requesting a mode the theme lacks, persists `/dark_mode` and `/display/theme` to settings.json, and disables the toggle outright for single-mode themes (a dark-only theme also force-switches the mode and hides the control). It publishes `settings_dark_mode` and `settings_dark_mode_available` subjects for the settings UI to bind; `Application` calls its `on_theme_changed()` once after both theme and settings are up so the toggle reflects what the loaded theme actually supports ([`src/application/application.cpp:873`](../../../src/application/application.cpp#L873)).
