...
4. [`src/application/application.cpp:711`](../../../src/application/application.cpp#L711) — `register_widgets()` (the first wave of C++ widget registrations), then `:1777` `register_xml_components()` and its hot-reloader wiring, and finally `:1960` the single `lv_xml_create` that instantiates the root layout. This is the whole boot ordering in four stops.
