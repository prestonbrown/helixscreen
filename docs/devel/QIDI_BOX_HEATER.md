...
3. **HelixScreen does not send the wrapper macros at all — the Phase 3 decision (#1022 de-stub plan) went to option (b), bypass.** The choice was between (a) ensuring/SETting `enable_box` + `value_t<n>` before issuing `T<n>`, and (b) driving the primitives directly. (b) shipped, for a reason stronger than the `enable_box` gate: on Q2 1.1.1 the `T0`-`T3` macros **do not exist** for the box's own slots (only `T4`+, for additional boxes), and they manage no hotend temperature. What HelixScreen emits today:
   - **Load** — `do_load_filament()` (`src/printer/ams_backend_qidi.cpp:1043`) sends `M109 S<temp>` → `EXTRUDER_LOAD SLOT=slot<n>` → `M104 S0`. `<temp>` is the slot's profile maximum (`nozzle_temp_max`, else `nozzle_temp_min`, else 250 °C), because Klipper rejects `EXTRUDER_LOAD` below `min_extrude_temp`. If a *different* slot is already loaded, its unload is prepended to the same sequence — `EXTRUDER_LOAD` on top of loaded filament jams. When the firmware has `CLEAR_NOZZLE` the wipe is appended and the whole sequence routes through `ensure_homed_then()`, because the stock macro makes absolute XY/Z moves with no homing guard of its own and a load from idle may be unhomed.
   - **Unload** — `do_unload_filament()` (`src/printer/ams_backend_qidi.cpp:1103`) prefers `M603 S<temp>`, the stock unload the QIDI screen itself runs. `UNLOAD_T<n>` and `UNLOAD_FILAMENT` both proved **inert** on Q2 firmware. Firmware without `M603` falls back to `M109` → `EXTRUDER_UNLOAD SLOT=slot<n>` → `M104 S0` (`build_unload_gcode()`, `src/printer/ams_backend_qidi.cpp:1029`). `M603` acts on whatever is in the extruder, so the slot index only selects the temperature.
   - **Tool change** — `do_change_tool()` (`src/printer/ams_backend_qidi.cpp:1152`) resolves tool → slot through the parsed `value_t<n>` mapping and delegates to the load path above. No `T<n>` is emitted.
