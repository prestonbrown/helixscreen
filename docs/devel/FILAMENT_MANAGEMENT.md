...
Every runout decision additionally requires the firmware to be *running* the sensor: Klipper's `filament_switch_sensor`/`filament_motion_sensor` status carries `enabled` (`SET_FILAMENT_SENSOR ENABLE=0/1`), and a stood-down sensor still reports `filament_detected` live but takes no runout action of its own, so `monitors_runout()` (config enabled + role + `state.enabled`) gates `has_any_runout()`/`has_real_runout()` and the edge toasts. The role subjects prefer a running holder and otherwise keep the first holder's live reading, so the sensor tile keeps displaying what the hardware reports while the decisions ignore it. A sensor that has never reported the field counts as running, and a Moonraker delta omitting it does not reset it (#1714).
