...
A worktree from `scripts/setup-worktree.sh` gets a PRIVATE checkout of `lib/helix-xml`
only. Everything else in `lib/`, `lib/lvgl` and `lib/libhv` included, is a symlink
shared with the main tree: never clean it from a worktree, and know that `patches/` is
per-branch while that checkout is not, so one shared checkout cannot serve two branches
with different patch sets (prestonbrown/helixscreen#1471).
