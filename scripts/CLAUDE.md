...
| `check_rotation_cache_order.py` | Fail if a `DisplayManager` function caches `m_width`/`m_height` from the LVGL resolution before `set_display_rotation()` settles: a scanout plane owning 90/270 un-swaps the resolution and the cache would record a value the display no longer has (#1587). A cache refresh with no settle call in the function (the external-resize catch-up) is legal. Source-order lint because `apply_rotation`'s body is `#ifndef HELIX_DISPLAY_SDL`-guarded out of the test binary. |
