...
| Script | Purpose |
|--------|---------|
| `setup-worktree.sh` | Create/configure git worktrees in `.worktrees/`. Symlinks the shared `lib/` submodules from the main tree and gives `lib/helix-xml` a private per-worktree checkout (`LIB_PRIVATE_SUBMODULES`): it is ours and edited directly, so a symlink would put every worktree's engine edits into the MAIN tree's submodule working copy. Gated by `tests/shell/test_worktree_setup.bats` |
