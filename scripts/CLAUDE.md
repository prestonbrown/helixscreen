...
| `doc_anchors.py` | Resolves a citation that names a place - `` `src/printer/printer_state.cpp#update_from_status` ``, quoted segments for names with spaces or overloads - to today's line, so no doc commits a line number. `--check` is advisory (`make check-doc-anchors`, from `.githooks/pre-push`); `--render <dir>` backs `make docs-pinned`. Tests: `tests/python/test_doc_anchors.py` |
...
| `check_icon_names.py` | Fail if an icon name written in `ui_xml/` (`<icon src="NAME">`, directly or through a component prop default or its instantiation) is absent from `include/ui_icon_codepoints.h`; a miss renders `image_broken_variant` with only a log warning. Always whole-tree, since deleting a codepoint breaks XML nowhere near the diff. `--summary` fails on an EMPTY corpus so a matcher that stops matching cannot read as green; `--list` prints every reference with its verdict |
...
| `check_esp32_override_drift.py` | Ratchet on `firmware/native-audit/overrides/` drifting from its `src/` twins (prestonbrown/helixscreen#1427): the forks are compiled instead of the twins by `app_srcs.txt` and nothing builds that tree, so a fix in `src/` silently reappears as a bug there. Compares content, not commit logs; every differing hunk must carry an `AUDIT OVERRIDE` marker on one of its own lines. A fork byte-identical to its twin is reported as redundant without failing |
...
| Script | Purpose |
|--------|---------|
| `setup-worktree.sh` | Create/configure git worktrees in `.worktrees/`. Symlinks the shared deps, builds fast. `LIB_PRIVATE_SUBMODULES` names the three that instead get a private per-worktree checkout: `lib/helix-xml` because it is ours and edited directly (never patched), so a symlink would put every worktree's engine edits into the MAIN tree's submodule working copy; `lib/lvgl` and `lib/libhv` because `patches/` rewrites them and `patches/` is per-branch, so one shared checkout cannot satisfy two branches with different patch sets and each tree's `make reapply-patches` silently redefines what the others compile (prestonbrown/helixscreen#1471). The private checkout is COPIED from the main tree (`cp -Rc`, mtimes preserved, so the cloned objects stay valid) with its git dir a `git clone --local` (hardlinked packs, no network), then `git checkout` reconciles this branch's pin, touching only the files that differ. Its gitdir lands under `.git/worktrees/<name>/modules/`, and it needs no `--unlink`/`--relink` since a real checkout is what git already expects. Setup ends by asserting every `lib/` submodule sits at this branch's pin and refuses to build when a private one does not. Gated by `tests/shell/test_worktree_setup.bats` |
