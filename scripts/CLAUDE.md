...
| `platform_manifest.py` | Read `assets/config/platforms.json` and answer derived questions about a platform - effective resolution after rotation, which splash classes its panel can select, printer image size. `mk/images.mk` calls it so `gen-splash-3d-<platform>` carries no resolution knowledge of its own; an unknown platform is a non-zero exit, never a silent fall-through to "generate everything" |
