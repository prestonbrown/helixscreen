...
| `check_ams_xml_mirror.py` | Fail if `AmsState::register_xml_subject_names()` (the re-entry mirror, #1374) and `AmsState::init_subjects()` publish different XML subject names (#1439). Reduces `INIT_SUBJECT_*` macros (a literal `false` publishes nothing), `register_subject_in_current_scope`, `lv_xml_register_subject` with a literal or constant name, and `snprintf(name_buf, ...)` loops keyed by their `for` bound. Fails closed: an unreducible registration call, a missing function or an empty extraction is a failure. No opt-out |
