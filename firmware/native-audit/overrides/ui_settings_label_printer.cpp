...
void LabelPrinterSettingsOverlay::handle_bt_printer_selected(int index) {
    ...
    if (!device.paired && !is_saved) {
        // Ported alongside the deleted lv_event_cb_t forms so this Phase-0
        // audit tree still compiles against the live headers. Mirrors the
        // real file's converted site: body inlined, MAC in the capture.
