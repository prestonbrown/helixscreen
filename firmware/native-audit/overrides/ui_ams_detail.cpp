...
void ams_detail_update_tray(AmsDetailWidgets& w) {
    ...
    int32_t depth = std::max<int32_t>(tray_height * DEPTH_PCT / 100, MIN_DEPTH_PX); // AUDIT OVERRIDE: int32_t=long on Xtensa
    int32_t dx = depth;
    int32_t dy = std::max<int32_t>(depth * DY_PCT / 100, MIN_DY_PX); // AUDIT OVERRIDE
