...
static void theme_manager_register_static_constants(lv_xml_component_scope_t* scope) {
    ...
    for (const auto& [name, value] :
         theme_manager_parse_all_xml_for_element(AUDIT_UI_XML_DIR, "px")) {
        if (!has_dynamic_suffix(name)) {
            lv_xml_register_const(scope, name.c_str(), value.c_str());
            px_count++;
        }
    ...
    for (const auto& [name, value] :
         theme_manager_parse_all_xml_for_element(AUDIT_UI_XML_DIR, "string")) {
        if (!has_dynamic_suffix(name)) {
            lv_xml_register_const(scope, name.c_str(), value.c_str());
            string_count++;
        }
...
void theme_manager_register_responsive_spacing(lv_display_t* display) {
    ...
    // horizontal resolution. Register first so auto-discovery silently skips duplicates.
    {
        ...
        auto nav_tokens =
            theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", nav_suffix);
    ...
    auto xxlarge_tokens =
        theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", "_xxlarge");
...
void theme_manager_register_responsive_fonts(lv_display_t* display) {
    ...
    auto micro_tokens =
        theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_micro");
    auto tiny_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_tiny");
    auto small_tokens =
        theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_small");
    auto medium_tokens =
        theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_medium");
    auto large_tokens =
        theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_large");
    auto xlarge_tokens =
        theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_xlarge");
    auto xxlarge_tokens =
        theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_xxlarge");
