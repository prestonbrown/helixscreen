...
// AUDIT OVERRIDE: ESP-IDF VFS has no working directory, so the desktop
// assumption cwd==install-root breaks every relative asset path. All
// responsive-token discovery scans go to the LittleFS mount instead.
// A Phase 2 port needs a real asset-root config (seam: relative paths).
#define AUDIT_UI_XML_DIR "/littlefs/ui_xml"
...
static void theme_manager_register_color_pairs(lv_xml_component_scope_t* scope, bool dark_mode) {
    ...
    auto light_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "color", "_light");
    auto dark_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "color", "_dark");
...
static void theme_manager_register_static_constants(lv_xml_component_scope_t* scope) {
    ...
    auto color_tokens = theme_manager_parse_all_xml_for_element(AUDIT_UI_XML_DIR, "color");
    ...
    for (const auto& [name, value] : theme_manager_parse_all_xml_for_element(AUDIT_UI_XML_DIR, "px")) {
        if (!has_dynamic_suffix(name)) {
            lv_xml_register_const(scope, name.c_str(), value.c_str());
            px_count++;
        }
    ...
    for (const auto& [name, value] : theme_manager_parse_all_xml_for_element(AUDIT_UI_XML_DIR, "string")) {
        if (!has_dynamic_suffix(name)) {
            lv_xml_register_const(scope, name.c_str(), value.c_str());
            string_count++;
        }
...
void theme_manager_register_responsive_spacing(lv_display_t* display) {
    ...
    // horizontal resolution. Register first so auto-discovery silently skips duplicates.
    {
        ...
        auto nav_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", nav_suffix);
    ...
    // and _xxlarge)
    auto micro_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", "_micro");
    auto tiny_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", "_tiny");
    auto small_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", "_small");
    auto medium_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", "_medium");
    auto large_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", "_large");
    auto xlarge_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", "_xlarge");
    auto xxlarge_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "px", "_xxlarge");
...
void theme_manager_register_responsive_fonts(lv_display_t* display) {
    ...
    auto micro_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_micro");
    auto tiny_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_tiny");
    auto small_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_small");
    auto medium_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_medium");
    auto large_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_large");
    auto xlarge_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_xlarge");
    auto xxlarge_tokens = theme_manager_parse_all_xml_for_suffix(AUDIT_UI_XML_DIR, "string", "_xxlarge");
...
// AUDIT OVERRIDE: the ~25 discovery passes each re-read every top-level XML
// file; on SPI-flash LittleFS that is minutes of boot time (watchdog-observed
// 150s+). Cache file bytes (goes to PSRAM via malloc) so flash is read once.
// Seam finding for a Phase 2 port: discovery must become single-pass/cached.
static const std::string& audit_cached_file_content(const char* filepath) {
    ...
    return cache.emplace(filepath, std::move(content)).first->second;
}
...
void theme_manager_parse_xml_file_for_all(
    ...
    const std::string& xml_content = audit_cached_file_content(filepath);
