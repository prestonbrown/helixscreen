...
/**
 * @brief Validate a Moonraker file root name.
 *
 * File roots are usually plain identifiers ("gcodes", "config"), but some
 * firmware exposes hidden roots such as ".temp". Keep this narrower than file
 * paths: a leading dot is allowed, but slashes, traversal, control characters,
 * and punctuation such as semicolons remain rejected.
 */
inline bool is_safe_file_root(const std::string& root) {
    ...
    return is_safe_identifier(root.substr(1));
}
...
/**
 * @brief Validate Moonraker file root and invoke error callback if invalid.
 */
inline bool reject_invalid_file_root(const std::string& root, const char* method,
                                     const MoonrakerAPI::ErrorCallback& on_error,
                                     bool silent = false) {
    if (is_safe_file_root(root)) {
        return false; // Valid, continue
    }

    if (!silent) {
        spdlog::error("[Moonraker API] {}: Invalid file root '{}'", method, root);
    }
