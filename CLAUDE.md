...
**Absolute for new code.** The tree still has 371 sites that break these rules
...
| Confirmation dialog | `modal_show_confirmation(title, msg, severity, btn_text, on_confirm, on_cancel, data)` (in `helix::ui`). Pushes with **no owner**, so a dismissal (backdrop tap, ESC, hot-reload rebuild) fires **neither** callback. Fine when the callbacks only act; wrong when the caller holds a guard/flag/pending entry they are meant to clear - subclass `Modal` there and resolve in `on_hide()` | `src/ui/ui_panel_macros.cpp:370`; subclass form: `include/lan_client_auth_router.h` |
